<?php

include('../../conect/conect.php');

$id_Proposta = intval($_GET['id']);

if(!isset($_SESSION))
    session_start();

function removerTextoDepoisDoEspaco($texto) {
   // Encontra a posição do primeiro espaço
   $posicaoPrimeiroEspaco = strpos($texto, ' ');

   // Se encontrar um espaço, encontra a posição do segundo espaço
   if ($posicaoPrimeiroEspaco !== false) {
       $posicaoSegundoEspaco = strpos($texto, ' ', $posicaoPrimeiroEspaco + 1);

       // Se encontrar o segundo espaço, remove o texto após o segundo espaço
       if ($posicaoSegundoEspaco !== false) {
           $texto = substr($texto, 0, $posicaoSegundoEspaco);
       }
   }

   return $texto;
}

function converterValorParaDecimal($valor) {
    $valor_sem_simbolo = str_replace("R$ ", "", $valor);
    $valor_sem_pontos = str_replace(".", "", $valor_sem_simbolo);
    $valor_com_ponto = str_replace(",", ".", $valor_sem_pontos);
    $valor_decimal = (float) $valor_com_ponto;

    return $valor_decimal;
}

function formatarValorParaReal($valor_decimal) {
    $valor_formatado = "R$ ";
    $valor_formatado .= number_format($valor_decimal, 2, ',', '.');
    return $valor_formatado;
}

function somarRendaBruta($rendaConj, $rendaComprador) {

    $valorComprador = converterValorParaDecimal($rendaComprador);
    $valorConjuge = converterValorParaDecimal($rendaConj);

    $somaRendas = $valorComprador + $valorConjuge;

    $valorFinal = formatarValorParaReal($somaRendas);

    return $valorFinal;

}

if (isset($_COOKIE['usuario']) || isset($_SESSION['usuario'])) {
    if (isset($_COOKIE['permissao_usuario']) && $_COOKIE['permissao_usuario'] == 5 || isset($_COOKIE['permissao_usuario']) && $_COOKIE['permissao_usuario'] == 0) {
        header("Location: ../../../logout.php");
        exit();
    }

    if (isset($_SESSION['permissao_usuario']) && $_SESSION['permissao_usuario'] == 5 || isset($_SESSION['permissao_usuario']) && $_SESSION['permissao_usuario'] == 0) {
        header("Location: ../../../logout.php");
        exit();
    }
} else {
    header("Location: ../../../login.php");
    exit();
}

if(isset($_COOKIE['usuario'])){
    $name=removerTextoDepoisDoEspaco($_COOKIE['nome']);
} else {
    $name=removerTextoDepoisDoEspaco($_SESSION['nome']);
}

if(isset($_COOKIE['usuario'])){
    $idUser = $_COOKIE['usuario'];
} elseif(isset($_SESSION['usuario'])){
    $idUser = $_SESSION['usuario'];
}

$sql_userPrimario = "SELECT * FROM usuarios WHERE id  = $idUser";
$query_userPrimario = $mysqli->query($sql_userPrimario) or die($mysqli->error);
$num_userPrimario = $query_userPrimario->num_rows;
$usuarioPrimario = $query_userPrimario->fetch_assoc();

$sql_dadosProposta = "SELECT * FROM db_propostas WHERE id_proposta = $id_Proposta";
$query_dadosProposta = $mysqli->query($sql_dadosProposta) or die($mysqli->error);
$num_dadosProposta = $query_dadosProposta->num_rows;
$dadosProposta = $query_dadosProposta->fetch_assoc();

$sql_dadosComprador = "SELECT * FROM db_comprador WHERE id_proposta = $id_Proposta";
$query_dadosComprador = $mysqli->query($sql_dadosComprador) or die($mysqli->error);
$num_dadosComprador = $query_dadosComprador->num_rows;

$sql_dadosVendedor = "SELECT * FROM db_vendedor WHERE id_proposta = $id_Proposta";
$query_dadosVendedor = $mysqli->query($sql_dadosVendedor) or die($mysqli->error);
$num_dadosVendedor = $query_dadosVendedor->num_rows;

$sql_dadosProcurador = "SELECT * FROM db_procurador WHERE id_proposta = $id_Proposta";
$query_dadosProcurador = $mysqli->query($sql_dadosProcurador) or die($mysqli->error);
$num_dadosProcurador = $query_dadosProcurador->num_rows;


if( $idUser != $dadosProposta['id_usuario']) {
    if( $idUser != $dadosProposta['id_MasterAcont']) {
        if($usuarioPrimario['permissao_usuario'] == 2 || $usuarioPrimario['permissao_usuario'] == 3){

        } else {    
            header("Location: ../../../logout.php");
            exit();        
        }
    } else {}
} else {}


?>


<!doctype html>
<html lang="pt-br">
  <head>
    <script src="../../../bootstrap/js/color-modes.js"></script>
    <script src="../../../javascript/jquery.min.js"></script>
    <script src="../../../javascript/jquery.mask.js"></script>
    <script src="../../../javascript/jquery.maskMoney.js"></script>

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Aprovacredi - Home</title>
    <link href="../../../bootstrap/dist/css/bootstrap.css" rel="stylesheet">
    <link rel="icon" href="../../../css/img/favicon.png" type="image/png">
    <link rel="stylesheet" href="../../../css/styleprimario.css">
    <link rel="stylesheet" href="../../../css/styleProposta.css">
    <link rel="stylesheet" href="../../../css/planicorretor.css">
    <link rel="stylesheet" href="../../../css/stylegestaoproposta.css">
  </head>
<body>
    <header style="position: relative;z-index: 9999;">
        <!--Navbar-->
        <div id="navbar">
            <nav class="navbar bg-body-ligt" style="background-color: #fff; box-shadow: rgba(0, 0, 0, 0.12) 0px 1px 3px, rgba(0, 0, 0, 0.24) 0px 1px 2px;">
                <div class="container">
                    <a class="navbar-brand" href="#">
                        <img src="../../../css/img/logo-Aprova.png" alt="Aprovacredi" width="150" height="60">
                    </a>


                    <div data-bs-toggle="offcanvas" href="#offcanvasExample" role="button" aria-controls="offcanvasExample" style="width: 5.5rem;">
                        <a href="#" class="d-block p-3 link-body-emphasis text-decoration-none" title="AprovaCredi" data-bs-toggle="tooltip" data-bs-placement="right">
                            <svg xmlns="http://www.w3.org/2000/svg" width="53" height="30" fill="currentColor" class="bi bi-justify" viewBox="0 0 16 16">
                                <path fill-rule="evenodd" d="M2 12.5a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5z"/>
                            </svg>      
                        <span class="visually-hidden">AprovaCredi</span>
                        </a>
                    </div>
                </div>
            </nav>
        </div>
        <!--Sidebar-->
        <div id="sidebar">
            <main class="d-flex flex-nowrap">
                <div class="d-flex flex-column flex-shrink-0" style="height: 100vh; background-color: #fff; box-shadow: rgba(0, 0, 0, 0.12) 0px 1px 3px, rgba(0, 0, 0, 0.24) 0px 1px 2px;">
                    <div data-bs-toggle="offcanvas" href="#offcanvasExample" role="button" aria-controls="offcanvasExample" style="width: 5.5rem;">
                        <a href="#" class="d-block p-3 link-body-emphasis text-decoration-none" title="AprovaCredi" data-bs-toggle="tooltip" data-bs-placement="right">
                            <svg xmlns="http://www.w3.org/2000/svg" width="53" height="30" fill="currentColor" class="bi bi-justify" viewBox="0 0 16 16">
                                <path fill-rule="evenodd" d="M2 12.5a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5z"/>
                            </svg>      
                        <span class="visually-hidden">AprovaCredi</span>
                        </a>
                    </div>
                    <hr style="margin:0px;">
                    <ul class="nav nav-pills nav-flush flex-column mb-auto text-center">
                        <!--Home-->    
                        <div class="dropend">
                            <!--Itens do Dropdown-->
                            <li class="nav-item"  data-bs-toggle="dropdown" aria-expanded="false">
                                <a href="#" class=" nav-link py-3 border-bottom rounded-0" aria-current="page" data-bs-toggle="tooltip" data-bs-placement="right">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                        <path d="M6.5 14.5v-3.505c0-.245.25-.495.5-.495h2c.25 0 .5.25.5.5v3.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0 0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293L8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5Z"/>
                                    </svg>
                                </a>
                            </li>
                            <!--Lista do Dropdown-->
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="../../../index.php" class=" dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                            <path d="M0 1a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V1zm9 0a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-5a1 1 0 0 1-1-1V1zm0 9a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-5a1 1 0 0 1-1-1v-5z"/>
                                        </svg>  
                                        Inicio
                                    </a>
                                </li>
                                <li>
                                    <a href="https://api.whatsapp.com/send?phone=5513991080129&text=Ol%C3%A1,%20preciso%20de%20ajuda%20com%20o%20sistema%20Aprovacredi!!" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z"/>
                                        </svg>    
                                        Ajuda
                                    </a>
                                </li>
                            </ul>
                        </div>

                        <!--Proposta-->    
                        <div class="dropend">
                            <li class="nav-item"  data-bs-toggle="dropdown" aria-expanded="false">
                                <a href="#" class="active nav-link py-3 border-bottom rounded-0" data-bs-toggle="tooltip" data-bs-placement="right">
                                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-cash-coin" viewBox="0 0 16 16">
                                    <path fill-rule="evenodd" d="M11 15a4 4 0 1 0 0-8 4 4 0 0 0 0 8zm5-4a5 5 0 1 1-10 0 5 5 0 0 1 10 0z"/>
                                    <path d="M9.438 11.944c.047.596.518 1.06 1.363 1.116v.44h.375v-.443c.875-.061 1.386-.529 1.386-1.207 0-.618-.39-.936-1.09-1.1l-.296-.07v-1.2c.376.043.614.248.671.532h.658c-.047-.575-.54-1.024-1.329-1.073V8.5h-.375v.45c-.747.073-1.255.522-1.255 1.158 0 .562.378.92 1.007 1.066l.248.061v1.272c-.384-.058-.639-.27-.696-.563h-.668zm1.36-1.354c-.369-.085-.569-.26-.569-.522 0-.294.216-.514.572-.578v1.1h-.003zm.432.746c.449.104.655.272.655.569 0 .339-.257.571-.709.614v-1.195l.054.012z"/>
                                    <path d="M1 0a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h4.083c.058-.344.145-.678.258-1H3a2 2 0 0 0-2-2V3a2 2 0 0 0 2-2h10a2 2 0 0 0 2 2v3.528c.38.34.717.728 1 1.154V1a1 1 0 0 0-1-1H1z"/>
                                    <path d="M9.998 5.083 10 5a2 2 0 1 0-3.132 1.65 5.982 5.982 0 0 1 3.13-1.567z"/>
                                </svg>
                                </a>
                            </li>
                            <!--Lista do Dropdown-->
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="../criacao/novaProposta.php" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                            <path fill-rule="evenodd" d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2Z"/>
                                        </svg>  
                                        Cadastrar Proposta
                                    </a>
                                </li>
                                <li>
                                    <a href="../gestao/myPropost.php" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M14.5 3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5h13zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
                                            <path d="M7 5.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5zm-1.496-.854a.5.5 0 0 1 0 .708l-1.5 1.5a.5.5 0 0 1-.708 0l-.5-.5a.5.5 0 1 1 .708-.708l.146.147 1.146-1.147a.5.5 0 0 1 .708 0zM7 9.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5zm-1.496-.854a.5.5 0 0 1 0 .708l-1.5 1.5a.5.5 0 0 1-.708 0l-.5-.5a.5.5 0 0 1 .708-.708l.146.147 1.146-1.147a.5.5 0 0 1 .708 0z"/>
                                        </svg>
                                        </svg>      
                                        Minhas Propostas
                                    </a>
                                </li>
                                <li>
                                    <a href="../gestao/admPropost.php" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M1 0 0 1l2.2 3.081a1 1 0 0 0 .815.419h.07a1 1 0 0 1 .708.293l2.675 2.675-2.617 2.654A3.003 3.003 0 0 0 0 13a3 3 0 1 0 5.878-.851l2.654-2.617.968.968-.305.914a1 1 0 0 0 .242 1.023l3.27 3.27a.997.997 0 0 0 1.414 0l1.586-1.586a.997.997 0 0 0 0-1.414l-3.27-3.27a1 1 0 0 0-1.023-.242L10.5 9.5l-.96-.96 2.68-2.643A3.005 3.005 0 0 0 16 3c0-.269-.035-.53-.102-.777l-2.14 2.141L12 4l-.364-1.757L13.777.102a3 3 0 0 0-3.675 3.68L7.462 6.46 4.793 3.793a1 1 0 0 1-.293-.707v-.071a1 1 0 0 0-.419-.814L1 0Zm9.646 10.646a.5.5 0 0 1 .708 0l2.914 2.915a.5.5 0 0 1-.707.707l-2.915-2.914a.5.5 0 0 1 0-.708ZM3 11l.471.242.529.026.287.445.445.287.026.529L5 13l-.242.471-.026.529-.445.287-.287.445-.529.026L3 15l-.471-.242L2 14.732l-.287-.445L1.268 14l-.026-.529L1 13l.242-.471.026-.529.445-.287.287-.445.529-.026L3 11Z"/>
                                        </svg>    
                                        Gerenciamento de Propostas
                                    </a>
                                </li>
                            </ul>
                        </div>

                        <!--Simulador-->  
                        <div class="dropend">
                            <li>
                                <a href="../../../simulador_apro.php" class="nav-link py-3 border-bottom rounded-0" title="Simulador" data-bs-toggle="tooltip" data-bs-placement="right">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-stack" viewBox="0 0 16 16">
                                        <path d="m14.12 10.163 1.715.858c.22.11.22.424 0 .534L8.267 15.34a.598.598 0 0 1-.534 0L.165 11.555a.299.299 0 0 1 0-.534l1.716-.858 5.317 2.659c.505.252 1.1.252 1.604 0l5.317-2.66zM7.733.063a.598.598 0 0 1 .534 0l7.568 3.784a.3.3 0 0 1 0 .535L8.267 8.165a.598.598 0 0 1-.534 0L.165 4.382a.299.299 0 0 1 0-.535L7.733.063z"/>
                                        <path d="m14.12 6.576 1.715.858c.22.11.22.424 0 .534l-7.568 3.784a.598.598 0 0 1-.534 0L.165 7.968a.299.299 0 0 1 0-.534l1.716-.858 5.317 2.659c.505.252 1.1.252 1.604 0l5.317-2.659z"/>
                                    </svg>
                                </a>
                            </li>
                        </div>

                        <!--Gestão de Usuarios-->
                        <div class="dropend">  
                            <li class="nav-item"  data-bs-toggle="dropdown" aria-expanded="false">
                                <a href="#" class="nav-link py-3 border-bottom rounded-0" data-bs-toggle="tooltip" data-bs-placement="right">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="45" height="35" fill="currentColor" class="bi bi-person-fill-gear" viewBox="0 0 16 16">
                                        <path d="M11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm-9 8c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Zm9.886-3.54c.18-.613 1.048-.613 1.229 0l.043.148a.64.64 0 0 0 .921.382l.136-.074c.561-.306 1.175.308.87.869l-.075.136a.64.64 0 0 0 .382.92l.149.045c.612.18.612 1.048 0 1.229l-.15.043a.64.64 0 0 0-.38.921l.074.136c.305.561-.309 1.175-.87.87l-.136-.075a.64.64 0 0 0-.92.382l-.045.149c-.18.612-1.048.612-1.229 0l-.043-.15a.64.64 0 0 0-.921-.38l-.136.074c-.561.305-1.175-.309-.87-.87l.075-.136a.64.64 0 0 0-.382-.92l-.148-.045c-.613-.18-.613-1.048 0-1.229l.148-.043a.64.64 0 0 0 .382-.921l-.074-.136c-.306-.561.308-1.175.869-.87l.136.075a.64.64 0 0 0 .92-.382l.045-.148ZM14 12.5a1.5 1.5 0 1 0-3 0 1.5 1.5 0 0 0 3 0Z"/>
                                    </svg>
                                </a>
                            </li>
                            <!--Lista do Dropdown-->
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="../../usuarios/add_user/solic_cadastro.php" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                            <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm.5-5v1h1a.5.5 0 0 1 0 1h-1v1a.5.5 0 0 1-1 0v-1h-1a.5.5 0 0 1 0-1h1v-1a.5.5 0 0 1 1 0Zm-2-6a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                            <path d="M2 13c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Z"/>                                
                                        </svg>  
                                        Solicitação de Cadastro
                                    </a>
                                </li>
                                <li>
                                    <a href="../../usuarios/add_user/user_registered.php" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm1.679-4.493-1.335 2.226a.75.75 0 0 1-1.174.144l-.774-.773a.5.5 0 0 1 .708-.708l.547.548 1.17-1.951a.5.5 0 1 1 .858.514ZM11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                            <path d="M2 13c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Z"/>                                </svg>
                                        </svg>      
                                        Usuarios Cadastrados
                                    </a>
                                </li>
                                <li>
                                    <a href="../../usuarios/add_user/user_bloqueado.php" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M13.879 10.414a2.501 2.501 0 0 0-3.465 3.465l3.465-3.465Zm.707.707-3.465 3.465a2.501 2.501 0 0 0 3.465-3.465Zm-4.56-1.096a3.5 3.5 0 1 1 4.949 4.95 3.5 3.5 0 0 1-4.95-4.95ZM11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm-9 8c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Z"/>
                                        </svg>    
                                        Usuarios Bloqueados
                                    </a>
                                </li>
                            </ul>
                        </div>

                        <!--Administração Geral-->
                        <div class="dropend">
                            <li class="nav-item"  data-bs-toggle="dropdown" aria-expanded="false">
                                <a href="#" class="nav-link py-3 border-bottom rounded-0" data-bs-toggle="tooltip" data-bs-placement="right">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-briefcase-fill" viewBox="0 0 16 16">
                                        <path d="M6.5 1A1.5 1.5 0 0 0 5 2.5V3H1.5A1.5 1.5 0 0 0 0 4.5v1.384l7.614 2.03a1.5 1.5 0 0 0 .772 0L16 5.884V4.5A1.5 1.5 0 0 0 14.5 3H11v-.5A1.5 1.5 0 0 0 9.5 1h-3zm0 1h3a.5.5 0 0 1 .5.5V3H6v-.5a.5.5 0 0 1 .5-.5z"/>
                                        <path d="M0 12.5A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5V6.85L8.129 8.947a.5.5 0 0 1-.258 0L0 6.85v5.65z"/>
                                    </svg>
                                </a>
                            </li>
                            <!--Lista do Dropdown-->
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="#" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                            <path d="M11 2a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v12h.5a.5.5 0 0 1 0 1H.5a.5.5 0 0 1 0-1H1v-3a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v3h1V7a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v7h1V2z"/>
                                        </svg>  
                                        Dashboard
                                    </a>
                                </li>
                                <li>
                                    <a href="#" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M3 0a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1h3v-3.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5V16h3a1 1 0 0 0 1-1V1a1 1 0 0 0-1-1H3Zm1 2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1Zm3 0a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1Zm3.5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5ZM4 5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1ZM7.5 5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5Zm2.5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1ZM4.5 8h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5Zm2.5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1Zm3.5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5Z"/>
                                        </svg>      
                                        Funcionarios
                                    </a>
                                </li>
                                <li>
                                    <a href="#" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M5.5 9.511c.076.954.83 1.697 2.182 1.785V12h.6v-.709c1.4-.098 2.218-.846 2.218-1.932 0-.987-.626-1.496-1.745-1.76l-.473-.112V5.57c.6.068.982.396 1.074.85h1.052c-.076-.919-.864-1.638-2.126-1.716V4h-.6v.719c-1.195.117-2.01.836-2.01 1.853 0 .9.606 1.472 1.613 1.707l.397.098v2.034c-.615-.093-1.022-.43-1.114-.9H5.5zm2.177-2.166c-.59-.137-.91-.416-.91-.836 0-.47.345-.822.915-.925v1.76h-.005zm.692 1.193c.717.166 1.048.435 1.048.91 0 .542-.412.914-1.135.982V8.518l.087.02z"/>
                                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                            <path d="M8 13.5a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11zm0 .5A6 6 0 1 0 8 2a6 6 0 0 0 0 12z"/>
                                        </svg>    
                                        Financeiro
                                    </a>
                                </li>
                                <li>
                                    <a href="#" class="dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                            <path d="M10 .5a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5.5.5 0 0 1-.5.5.5.5 0 0 0-.5.5V2a.5.5 0 0 0 .5.5h5A.5.5 0 0 0 11 2v-.5a.5.5 0 0 0-.5-.5.5.5 0 0 1-.5-.5Z"/>
                                            <path d="M4.085 1H3.5A1.5 1.5 0 0 0 2 2.5v12A1.5 1.5 0 0 0 3.5 16h9a1.5 1.5 0 0 0 1.5-1.5v-12A1.5 1.5 0 0 0 12.5 1h-.585c.055.156.085.325.085.5V2a1.5 1.5 0 0 1-1.5 1.5h-5A1.5 1.5 0 0 1 4 2v-.5c0-.175.03-.344.085-.5ZM9.98 5.356 11.372 10h.128a.5.5 0 0 1 0 1H11a.5.5 0 0 1-.479-.356l-.94-3.135-1.092 5.096a.5.5 0 0 1-.968.039L6.383 8.85l-.936 1.873A.5.5 0 0 1 5 11h-.5a.5.5 0 0 1 0-1h.191l1.362-2.724a.5.5 0 0 1 .926.08l.94 3.135 1.092-5.096a.5.5 0 0 1 .968-.039Z"/>                                
                                        </svg>    
                                        Relatorio
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </ul>
                    <div class="dropdown border-top">
                        <a href="#" class="d-flex align-items-center justify-content-center p-3 link-body-emphasis text-decoration-none dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                            <img 
                            <?php if($usuarioPrimario['nome_arq_ftn_Perfil'] == null) { ?> 
                               src="../../../css/img/fotoperfil.jpg"  
                            <?php } else 
                            { echo 'src="../../usuarios/'.$usuarioPrimario["path_ftn_perfil"].'"';}?> 
                            alt="mdo" width="35" height="35" class="rounded-circle">
                        </a>
                        <ul class="dropdown-menu text-small shadow">
                            <li><a class=" dropdown-item" href="../../usuarios/conta_usuario/editar_user_acont.php">Perfil</a></li>
                            <li><a class="dropdown-item" href="../../usuarios/conta_usuario/creact_sec_login.php">Cadastro Corretor</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item" href="../../../logout.php">Sair</a></li>
                        </ul>
                    </div>
                </div>
            </main>
        </div>
        <!--Menu Lateral-->
        <div class="offcanvas offcanvas-start"  id="offcanvasExample" aria-labelledby="offcanvasExampleLabel" style="width: 280px;">
                    <div class="d-flex flex-column flex-shrink-0 p-3 bg-body-light" style="width: 279px; height: 100%;">
                        <img src="../../../css/img/logo-Aprova.png" alt="">
                        <hr>
                        <ul class="nav nav-pills flex-column mb-auto mobile">
                            <!--Home Lateral-->  
                            <div class="dropend">
                                <li class="nav-item dropdown">
                                    <a href="#" class="nav-link link-body-emphasis" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                        <path d="M6.5 14.5v-3.505c0-.245.25-.495.5-.495h2c.25 0 .5.25.5.5v3.5a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.146-.354L13 5.793V2.5a.5.5 0 0 0-.5-.5h-1a.5.5 0 0 0-.5.5v1.293L8.354 1.146a.5.5 0 0 0-.708 0l-6 6A.5.5 0 0 0 1.5 7.5v7a.5.5 0 0 0 .5.5h4a.5.5 0 0 0 .5-.5Z"/>
                                    </svg>                                        
                                    Home
                                    </a>
                                    <!--Lista do Dropdown-->
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a href="../../../index.php" class="dropdown-item" >
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                                    <path d="M0 1a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V1zm9 0a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-5a1 1 0 0 1-1-1V1zm0 9a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1h-5a1 1 0 0 1-1-1v-5z"/>
                                                </svg>  
                                                Inicio
                                            </a>
                                        </li>
                                        <li>
                                            <a href="https://api.whatsapp.com/send?phone=5513991080129&text=Ol%C3%A1,%20preciso%20de%20ajuda%20com%20o%20sistema%20Aprovacredi!!" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z"/>
                                                </svg>    
                                                Ajuda
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                            </div>

                            <!--Propostas Lateral-->  
                            <div class="dropend">
                                <li class="nav-item dropdown">
                                    <a href="#" class=" active nav-link link-body-emphasis" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-cash-coin" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M11 15a4 4 0 1 0 0-8 4 4 0 0 0 0 8zm5-4a5 5 0 1 1-10 0 5 5 0 0 1 10 0z"/>
                                        <path d="M9.438 11.944c.047.596.518 1.06 1.363 1.116v.44h.375v-.443c.875-.061 1.386-.529 1.386-1.207 0-.618-.39-.936-1.09-1.1l-.296-.07v-1.2c.376.043.614.248.671.532h.658c-.047-.575-.54-1.024-1.329-1.073V8.5h-.375v.45c-.747.073-1.255.522-1.255 1.158 0 .562.378.92 1.007 1.066l.248.061v1.272c-.384-.058-.639-.27-.696-.563h-.668zm1.36-1.354c-.369-.085-.569-.26-.569-.522 0-.294.216-.514.572-.578v1.1h-.003zm.432.746c.449.104.655.272.655.569 0 .339-.257.571-.709.614v-1.195l.054.012z"/>
                                        <path d="M1 0a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h4.083c.058-.344.145-.678.258-1H3a2 2 0 0 0-2-2V3a2 2 0 0 0 2-2h10a2 2 0 0 0 2 2v3.528c.38.34.717.728 1 1.154V1a1 1 0 0 0-1-1H1z"/>
                                        <path d="M9.998 5.083 10 5a2 2 0 1 0-3.132 1.65 5.982 5.982 0 0 1 3.13-1.567z"/>
                                    </svg>                                        
                                    Propostas
                                    </a>
                                    <!--Lista do Dropdown-->
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a href="../criacao/novaProposta.php" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                                    <path fill-rule="evenodd" d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2Z"/>
                                                </svg>  
                                                Cadastrar Proposta
                                            </a>
                                        </li>
                                        <li>
                                            <a href="myPropost.php" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M14.5 3a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5h13zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
                                                    <path d="M7 5.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5zm-1.496-.854a.5.5 0 0 1 0 .708l-1.5 1.5a.5.5 0 0 1-.708 0l-.5-.5a.5.5 0 1 1 .708-.708l.146.147 1.146-1.147a.5.5 0 0 1 .708 0zM7 9.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5zm-1.496-.854a.5.5 0 0 1 0 .708l-1.5 1.5a.5.5 0 0 1-.708 0l-.5-.5a.5.5 0 0 1 .708-.708l.146.147 1.146-1.147a.5.5 0 0 1 .708 0z"/>
                                                </svg>
                                                </svg>      
                                                Minhas Propostas
                                            </a>
                                        </li>
                                        <li>
                                            <a href="admPropost.php" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M1 0 0 1l2.2 3.081a1 1 0 0 0 .815.419h.07a1 1 0 0 1 .708.293l2.675 2.675-2.617 2.654A3.003 3.003 0 0 0 0 13a3 3 0 1 0 5.878-.851l2.654-2.617.968.968-.305.914a1 1 0 0 0 .242 1.023l3.27 3.27a.997.997 0 0 0 1.414 0l1.586-1.586a.997.997 0 0 0 0-1.414l-3.27-3.27a1 1 0 0 0-1.023-.242L10.5 9.5l-.96-.96 2.68-2.643A3.005 3.005 0 0 0 16 3c0-.269-.035-.53-.102-.777l-2.14 2.141L12 4l-.364-1.757L13.777.102a3 3 0 0 0-3.675 3.68L7.462 6.46 4.793 3.793a1 1 0 0 1-.293-.707v-.071a1 1 0 0 0-.419-.814L1 0Zm9.646 10.646a.5.5 0 0 1 .708 0l2.914 2.915a.5.5 0 0 1-.707.707l-2.915-2.914a.5.5 0 0 1 0-.708ZM3 11l.471.242.529.026.287.445.445.287.026.529L5 13l-.242.471-.026.529-.445.287-.287.445-.529.026L3 15l-.471-.242L2 14.732l-.287-.445L1.268 14l-.026-.529L1 13l.242-.471.026-.529.445-.287.287-.445.529-.026L3 11Z"/>
                                                </svg>    
                                                Gerenciamento de Propostas
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                            </div>

                            <!--Simulador Lateral-->  
                            <li>
                                <a href="../../../simulador_apro.php" class="nav-link link-body-emphasis">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-stack" viewBox="0 0 16 16">
                                        <path d="m14.12 10.163 1.715.858c.22.11.22.424 0 .534L8.267 15.34a.598.598 0 0 1-.534 0L.165 11.555a.299.299 0 0 1 0-.534l1.716-.858 5.317 2.659c.505.252 1.1.252 1.604 0l5.317-2.66zM7.733.063a.598.598 0 0 1 .534 0l7.568 3.784a.3.3 0 0 1 0 .535L8.267 8.165a.598.598 0 0 1-.534 0L.165 4.382a.299.299 0 0 1 0-.535L7.733.063z"/>
                                        <path d="m14.12 6.576 1.715.858c.22.11.22.424 0 .534l-7.568 3.784a.598.598 0 0 1-.534 0L.165 7.968a.299.299 0 0 1 0-.534l1.716-.858 5.317 2.659c.505.252 1.1.252 1.604 0l5.317-2.659z"/>
                                    </svg>                                    
                                    Simulador
                                </a>
                            </li>

                            <!--Gestão de Usuarios Lateral-->  
                            <div class="dropend">
                                <li class="nav-item dropdown">
                                    <a href="#" class="nav-link link-body-emphasis" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="45" height="35" fill="currentColor" class="bi bi-person-fill-gear" viewBox="0 0 16 16">
                                            <path d="M11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm-9 8c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Zm9.886-3.54c.18-.613 1.048-.613 1.229 0l.043.148a.64.64 0 0 0 .921.382l.136-.074c.561-.306 1.175.308.87.869l-.075.136a.64.64 0 0 0 .382.92l.149.045c.612.18.612 1.048 0 1.229l-.15.043a.64.64 0 0 0-.38.921l.074.136c.305.561-.309 1.175-.87.87l-.136-.075a.64.64 0 0 0-.92.382l-.045.149c-.18.612-1.048.612-1.229 0l-.043-.15a.64.64 0 0 0-.921-.38l-.136.074c-.561.305-1.175-.309-.87-.87l.075-.136a.64.64 0 0 0-.382-.92l-.148-.045c-.613-.18-.613-1.048 0-1.229l.148-.043a.64.64 0 0 0 .382-.921l-.074-.136c-.306-.561.308-1.175.869-.87l.136.075a.64.64 0 0 0 .92-.382l.045-.148ZM14 12.5a1.5 1.5 0 1 0-3 0 1.5 1.5 0 0 0 3 0Z"/>
                                        </svg>                                    
                                        Gestão de Usuarios
                                    </a>
                                    <!--Lista do Dropdown-->
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a href="../../usuarios/add_user/solic_cadastro.php" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                                    <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm.5-5v1h1a.5.5 0 0 1 0 1h-1v1a.5.5 0 0 1-1 0v-1h-1a.5.5 0 0 1 0-1h1v-1a.5.5 0 0 1 1 0Zm-2-6a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                                    <path d="M2 13c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Z"/>                                
                                                </svg>  
                                                Solicitação de Cadastro
                                            </a>
                                        </li>
                                        <li>
                                            <a href="../../usuarios/add_user/user_registered.php" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm1.679-4.493-1.335 2.226a.75.75 0 0 1-1.174.144l-.774-.773a.5.5 0 0 1 .708-.708l.547.548 1.17-1.951a.5.5 0 1 1 .858.514ZM11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                                    <path d="M2 13c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Z"/>                                </svg>
                                                </svg>      
                                                Usuarios Cadastrados
                                            </a>
                                        </li>
                                        <li>
                                            <a href="../../usuarios/add_user/user_bloqueado.php" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M13.879 10.414a2.501 2.501 0 0 0-3.465 3.465l3.465-3.465Zm.707.707-3.465 3.465a2.501 2.501 0 0 0 3.465-3.465Zm-4.56-1.096a3.5 3.5 0 1 1 4.949 4.95 3.5 3.5 0 0 1-4.95-4.95ZM11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm-9 8c0 1 1 1 1 1h5.256A4.493 4.493 0 0 1 8 12.5a4.49 4.49 0 0 1 1.544-3.393C9.077 9.038 8.564 9 8 9c-5 0-6 3-6 4Z"/>
                                                </svg>    
                                                Usuarios Bloqueados
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                            </div>

                            <!--Administração Lateral-->  
                            <div class="dropend">
                                <li class="nav-item dropdown">
                                    <a href="#" class="nav-link link-body-emphasis" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" fill="currentColor" class="bi bi-briefcase-fill" viewBox="0 0 16 16">
                                            <path d="M6.5 1A1.5 1.5 0 0 0 5 2.5V3H1.5A1.5 1.5 0 0 0 0 4.5v1.384l7.614 2.03a1.5 1.5 0 0 0 .772 0L16 5.884V4.5A1.5 1.5 0 0 0 14.5 3H11v-.5A1.5 1.5 0 0 0 9.5 1h-3zm0 1h3a.5.5 0 0 1 .5.5V3H6v-.5a.5.5 0 0 1 .5-.5z"/>
                                            <path d="M0 12.5A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5V6.85L8.129 8.947a.5.5 0 0 1-.258 0L0 6.85v5.65z"/>
                                        </svg>                                   
                                        Administração
                                    </a>
                                    <!--Lista do Dropdown-->
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a href="#" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-grid-1x2-fill" viewBox="0 0 16 16">
                                                    <path d="M11 2a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v12h.5a.5.5 0 0 1 0 1H.5a.5.5 0 0 1 0-1H1v-3a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v3h1V7a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v7h1V2z"/>
                                                </svg>  
                                                Dashboard
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M3 0a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1h3v-3.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5V16h3a1 1 0 0 0 1-1V1a1 1 0 0 0-1-1H3Zm1 2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1Zm3 0a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1Zm3.5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5ZM4 5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1ZM7.5 5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5Zm2.5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1ZM4.5 8h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5Zm2.5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1Zm3.5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5Z"/>
                                                </svg>      
                                                Funcionarios
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M5.5 9.511c.076.954.83 1.697 2.182 1.785V12h.6v-.709c1.4-.098 2.218-.846 2.218-1.932 0-.987-.626-1.496-1.745-1.76l-.473-.112V5.57c.6.068.982.396 1.074.85h1.052c-.076-.919-.864-1.638-2.126-1.716V4h-.6v.719c-1.195.117-2.01.836-2.01 1.853 0 .9.606 1.472 1.613 1.707l.397.098v2.034c-.615-.093-1.022-.43-1.114-.9H5.5zm2.177-2.166c-.59-.137-.91-.416-.91-.836 0-.47.345-.822.915-.925v1.76h-.005zm.692 1.193c.717.166 1.048.435 1.048.91 0 .542-.412.914-1.135.982V8.518l.087.02z"/>
                                                    <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                                    <path d="M8 13.5a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11zm0 .5A6 6 0 1 0 8 2a6 6 0 0 0 0 12z"/>
                                                </svg>    
                                                Financeiro
                                            </a>
                                        </li>
                                        <li>
                                            <a href="#" class="dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M10 .5a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5.5.5 0 0 1-.5.5.5.5 0 0 0-.5.5V2a.5.5 0 0 0 .5.5h5A.5.5 0 0 0 11 2v-.5a.5.5 0 0 0-.5-.5.5.5 0 0 1-.5-.5Z"/>
                                                    <path d="M4.085 1H3.5A1.5 1.5 0 0 0 2 2.5v12A1.5 1.5 0 0 0 3.5 16h9a1.5 1.5 0 0 0 1.5-1.5v-12A1.5 1.5 0 0 0 12.5 1h-.585c.055.156.085.325.085.5V2a1.5 1.5 0 0 1-1.5 1.5h-5A1.5 1.5 0 0 1 4 2v-.5c0-.175.03-.344.085-.5ZM9.98 5.356 11.372 10h.128a.5.5 0 0 1 0 1H11a.5.5 0 0 1-.479-.356l-.94-3.135-1.092 5.096a.5.5 0 0 1-.968.039L6.383 8.85l-.936 1.873A.5.5 0 0 1 5 11h-.5a.5.5 0 0 1 0-1h.191l1.362-2.724a.5.5 0 0 1 .926.08l.94 3.135 1.092-5.096a.5.5 0 0 1 .968-.039Z"/>                                
                                                </svg>    
                                                Relatorio
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                            </div>
                        </ul>
                        <hr>
                        <div class="dropdown">
                            <a href="#" class="d-flex align-items-center link-body-emphasis text-decoration-none dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                                <img 
                                <?php if($usuarioPrimario['nome_arq_ftn_Perfil'] == null) { ?> 
                                    src="css/img/fotoperfil.jpg"  
                                <?php } else { echo 'src="../../usuarios/'.$usuarioPrimario["path_ftn_perfil"].'"';}?>
                                alt="" width="32" height="35" class="rounded-circle me-2">
                                <strong><?php echo "$name" ?></strong>
                            </a>
                            <ul class="dropdown-menu text-small shadow">
                                <li><a class="dropdown-item" href="../../usuarios/conta_usuario/editar_user_acont.php">Perfil</a></li>
                                <li><a class="dropdown-item" href="../../usuarios/conta_usuario/creact_sec_login.php">Cadastro Corretor</a></li>
                                <li><hr class="dropdown-divider"></li>
                                <li><a class="dropdown-item" href="../../../logout.php">Sair</a></li>
                            </ul>
                        </div>
                    </div>
        </div>
        </div>
    </header>
    <section class="workspace">
            <div class="container container-workspace containerdados">
                <!--Conteudo da Pagina-->
                <?php if($usuarioPrimario['permissao_usuario'] == 3 || $usuarioPrimario['permissao_usuario'] == 2) {?>
                <div class="headerDados">
                    <h1><b>Proposta de <?php echo removerTextoDepoisDoEspaco($dadosProposta['nome_comprador']); ?></b></h1>
                    <hr>
                </div>

            
                <div class="containerDadosProposta">
                    <div class="">
                        <form action="altStatusProp.php" method="post">
                        <div class="movPropsArea row">
                            <div class="col-6">
                                <div class="inputStatusProposta">
                                    <label class="form-label">Status da proposta</label>
                                    <select name="statusProposta" class="form-select" aria-label="Default select example">
                                        <option value="<?php echo $dadosProposta['status_proposta']; ?>"><?php echo $dadosProposta['status_proposta']; ?></option>
                                        <option value="Reprovado">Reprovado</option>
                                        <option value="Aprovado">Aprovado</option>
                                        <option value="Pre Aprovado">Pre Aprovado</option>
                                        <option value="Aguardando documentação do Imovel">Aguardando Documentação do Imovel</option>
                                        <option value="Aguardando documentação do Vendedor">Aguardando Documentação do Vendedor</option>
                                        <option value="Aprovado: Mediante Comprovação de Renda">Aprovado: Mediante Comprovação de Renda</option>
                                        <option value="Aprovado: Aguardando documento do vendedor">Aprovado: Aguardando documento do vendedor</option>
                                        <option value="Aguardando Engenharia">Aguardando Engenharia</option>
                                        <option value="Engenharia Reprovada - Revisão de Laudo">Engenharia Reprovada - Revisão de Laudo</option>
                                        <option value="Elaborando Formularios">Elaborando Formularios</option>
                                        <option value="Elaborando Formularios - Saque do FGTS">Elaborando Formularios - Saque do FGTS</option>
                                        <option value="Aguardando Assinatura dos Formularios">Aguardando Assinatura dos Formularios</option>
                                        <option value="Elaborando SIOP e SICTD">Elaborando SIOP e SICTD</option>
                                        <option value="Conformidade - Aguardando">Conformidade - Aguardando</option>
                                        <option value="Assinatura Feita">Assinatura Feita</option>
                                        <option value="Laudo aprovado - Formação do Contrato">Laudo aprovado - Formação de Contrato</option>
                                        <option value="Laudo reprovado">Laudo reprovado</option>
                                        <option value="Laudo em reanálise">Laudo em reanálise</option>
                                        <option value="Aguardando emissão de minuta">Aguardando emissão de minuta</option>
                                        <option value="Aguardando retorno do registro">Aguardando retorno do registro</option>
                                        <option value="Aprovado: Valor liberado ao vendedor">Aprovado: Valor liberado ao vendedor</option>
                                        <option value="Finalizado">Finalizado</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="inputStatusProposta">
                                    <label class="form-label">Banco</label>
                                    <select name="bancoAprovador" class="form-select" aria-label="Default select example">
                                        <option value="N/A">Selecione o Banco</option>
                                        <option value="Itau">Itau</option>
                                        <option value="Bradesco">Bradesco</option>
                                        <option value="Santander">Santander</option>
                                        <option value="Caixa-SBPE">Caixa - SBPE</option>
                                        <option value="Caixa-PMCMV">Caixa - PMCMV</option>
                                        <option value="Caixa-PRO-COTISTA">Caixa - PRO-COTISTA</option>
                                        <option value="Inter">Inter</option>
                                    </select>
                                </div>
                            </div>
                            <input type="hidden" name="idProposta" value="<?php echo $id_Proposta;?>">
                            <input type="hidden" name="idUser" value="<?php echo $dadosProposta['id_usuario'];?>">
                            <div class="btnStatusProposta">
                                <button style="margin-left:auto;" type="submit" class="cta">
                                    <span class="hover-underline-animation">Salvar</span>
                                    <svg
                                        id="arrow-horizontal"
                                        xmlns="http://www.w3.org/2000/svg"
                                        width="30"
                                        height="10"
                                        viewBox="0 0 46 16"
                                    >
                                        <path
                                        id="Path_10"
                                        data-name="Path 10"
                                        d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                        transform="translate(30)"
                                        ></path>
                                    </svg>
                                </button>
                            </div>
                        </div>

                        </form>
                    </div>

                </div>
                <?php } else {} ?>
                    <div class="containerInfImovel">
                            <div class="headerSubDados">
                                <h1><b>Dados Imovel</b></h1>
                                <hr>
                            </div>
                            <form class="formDados" action="ditarDadosImovel.php?id=<?php echo $dadosProposta['id_proposta']; ?>" method="post">
                                <div class="infoArea row">
                                    <div class="disabledInput col-md-4 col-sm-12">
                                        <label for="DescrImovel" class="form-label">Descrição do Imovel</label>
                                        <input name="DescrImovel"  disabled value="<?php echo $dadosProposta['descrição_imovel']; ?>" type="text" id="DescrImovel" class="form-control">
                                    </div>

                                    <div class="disabledInput col-md-4 col-sm-12">
                                        <label for="MatImovel" class="form-label">Matrícula Imovel</label>
                                        <input name="MatImovel" disabled value="<?php echo $dadosProposta['matricula_imovel']; ?>" type="text" id="MatImovel" class="form-control">
                                    </div>

                                    <div class="col-md-4 col-sm-12">
                                        <label for="bancoAprovador" class="form-label">Banco Aprovador</label>
                                        <input name="bancoAprovador" disabled value="<?php echo $dadosProposta['banco_aprov']; ?>" type="text" id="bancoAprovador" class="form-control">
                                    </div>
                                </div>
                                <div class="infoArea row">
                                    <div class="disabledInput col-md-4 col-sm-12">
                                        <label for="cepInput" class="form-label">CEP</label>
                                        <input id="cepInput" disabled value="<?php echo $dadosProposta['cep_home']; ?>" name="cepuser"  type="text" class="form-control">
                                    </div>

                                    <div class="disabledInput col-md-6 col-sm-12">
                                        <label for="logradouroHome" class="form-label">Logradouro</label>
                                        <input name="ruauser"  disabled value="<?php echo $dadosProposta['endereço_imovel']; ?>" type="text" id="logradouroHome" class="form-control">
                                    </div>

                                    <div class="disabledInput col-md-2 col-sm-12">
                                        <label for="numberHome" class="form-label">Numero</label>
                                        <input name="numberHome"  disabled value="<?php echo $dadosProposta['number_home']; ?>" type="text" id="numberHome" class="form-control">
                                    </div>
                                </div>
                                <div class="infoArea row">
                                    <div class="disabledInput col-md-5 col-sm-12">
                                        <label for="bairroImovel" class="form-label">Bairro</label>
                                        <input name="bairrouser" disabled value="<?php echo $dadosProposta['bairro_imovel']; ?>" type="text" id="bairroImovel" class="form-control">
                                    </div>

                                    <div class="col-md-5 col-sm-12">
                                        <label for="cidadeImovel" class="form-label">Cidade</label>
                                        <input name="cidadeuser"  disabled value="<?php echo $dadosProposta['cidade_imovel']; ?>" type="text" id="cidadeImovel" class="form-control">
                                    </div>

                                    <div class="col-md-2 col-sm-12">
                                        <label for="estadoImovel" class="form-label">Estado</label>
                                        <input name="estadouser"  disabled  value="<?php echo $dadosProposta['estado_imovel']; ?>" type="text" id="estadoImovel" class="form-control">
                                    </div>

                                </div>
                                <div class="infoArea row">
                                    <div class="disabledInput col-md-3 col-sm-12">
                                        <label class="form-label">Tipo de Imovel</label>
                                        <select name="selectType" class="form-select" aria-label="Disabled select example" disabled>
                                            <option selected value="<?php echo $dadosProposta['tipo_imovel']; ?>"><?php if($dadosProposta['tipo_imovel'] == "" || $dadosProposta['tipo_imovel'] == "NULL") { echo "Selecione um tipo";} else {echo $dadosProposta['tipo_imovel'];} ?></option>
                                            <option value="Novo">Novo</option>
                                            <option value="Usado">Usado</option>
                                        </select>
                                    </div>

                                    <div class="disabledInput col-md-3 col-sm-12">
                                        <label for="valorImovel" class="form-label">Valor Imovel</label>
                                        <input name="valorImovel" disabled  value="<?php echo $dadosProposta['valor_imovel']; ?>" type="text" id="valorImovel" class="formatReal form-control">
                                    </div>

                                    <div class="disabledInput col-md-3 col-sm-12">
                                        <label for="valorFinanciado" class="form-label">Valor Financiado</label>
                                        <input name="valorFinanciado" disabled value="<?php echo $dadosProposta['financiado']; ?>" type="text" id="valorFinanciado" class="formatReal form-control">
                                    </div>

                                    <div class="disabledInput col-md-3 col-sm-12">
                                        <label for="fgtsImovel" class="form-label">FGTS a ser Utilizado</label>
                                        <input name="fgtsImovel" disabled value="<?php echo $dadosProposta['valor_fgts']; ?>" type="text" id="fgtsImovel" class="formatReal form-control">
                                    </div>
                                </div>

                                <div class="btnStatusProposta">
                                    <button id="btnDisableInput"  style="margin-left:auto;" class="cta">
                                        <span class="hover-underline-animation">Editar dados</span>
                                        <svg
                                            id="arrow-horizontal"
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="30"
                                            height="10"
                                            viewBox="0 0 46 16"
                                        >
                                            <path
                                            id="Path_10"
                                            data-name="Path 10"
                                            d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                            transform="translate(30)"
                                            ></path>
                                        </svg>
                                    </button>
                                </div>
                            </form>
                        </div>
                        <div class="containerInfImovel">
                        <!--Tabela Comprador-->
                            <div class="compradorArea">
                                <div class="">
                                    <h1><b>Dados Comprador</b></h1>
                                    <hr>
                                </div>  
                                <table class="table-proposta table">
                                    <thead>
                                        <tr scope="">
                                            <th class="" >Nome</th>
                                            <th class="" >CPF</th>
                                            <th class="">Estado Civil</th>
                                            <th class="">Renda Bruta</th>
                                            <th class=""></th>
                                        </tr>
                                    </thead>
                                        <?php if($num_dadosComprador == 0) { ?>
                                                            <tr>
                                                                <td colspan="6" style="text-align: center;">Nem um comprador encontrado!</td>
                                                            </tr>
                                        <?php } else { 
                                            while($compradorProposta = $query_dadosComprador->fetch_assoc()){
                                            ?>
                                    <tbody>
                                        <tr scope="row">
                                            <th class="">
                                                <div class="">
                                                    <span class=""><?php echo removerTextoDepoisDoEspaco($compradorProposta['nome_comprador']); ?></span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class=""><?php echo $compradorProposta['cpf_comprador']; ?></span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class=""><?php echo $compradorProposta['estado_civil__comprador']; ?></span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class=""><?php echo somarRendaBruta($compradorProposta['renda_bruta_conjugue'], $compradorProposta['renda_bruta_comprador']); ?></span>
                                                </div>
                                            </th>
                                            <th class=" edicaoProposta" style="text-align: right;">
                                                <button class="buttonEditar" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 20 20" height="20" fill="none" class="svg-icon">
                                                        <g stroke-width="1.5" stroke-linecap="round" stroke="#4e4e4e">
                                                            <circle r="2.5" cy="10" cx="10"></circle>
                                                            <path fill-rule="evenodd" d="m8.39079 2.80235c.53842-1.51424 2.67991-1.51424 3.21831-.00001.3392.95358 1.4284 1.40477 2.3425.97027 1.4514-.68995 2.9657.82427 2.2758 2.27575-.4345.91407.0166 2.00334.9702 2.34248 1.5143.53842 1.5143 2.67996 0 3.21836-.9536.3391-1.4047 1.4284-.9702 2.3425.6899 1.4514-.8244 2.9656-2.2758 2.2757-.9141-.4345-2.0033.0167-2.3425.9703-.5384 1.5142-2.67989 1.5142-3.21831 0-.33914-.9536-1.4284-1.4048-2.34247-.9703-1.45148.6899-2.96571-.8243-2.27575-2.2757.43449-.9141-.01669-2.0034-.97028-2.3425-1.51422-.5384-1.51422-2.67994.00001-3.21836.95358-.33914 1.40476-1.42841.97027-2.34248-.68996-1.45148.82427-2.9657 2.27575-2.27575.91407.4345 2.00333-.01669 2.34247-.97026z" clip-rule="evenodd">
                                                            </path>
                                                        </g>
                                                    </svg>
                                                </button>
                                                <ul class="dropdown-menu">
                                                    <li><a class="dropdown-item" href="editarComprador.php?id=<?php echo $compradorProposta['id_comprador']; ?>">Editar Comprador</a></li>
                                                <?php if($usuarioPrimario['permissao_usuario'] == 3 || $usuarioPrimario['permissao_usuario'] == 2) {?> <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalDocComp<?php echo $compradorProposta['id_comprador']; ?>" <?php echo $compradorProposta['id_comprador']; ?>>Documentos Comprador</a></li> <?php } else {} ?>
                                                    <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalExcluirComprador<?php echo $compradorProposta['id_comprador']; ?>">Excluir Comprador</a></li>
                                                </ul>
                                            </th>
                                        </tr>
                                    </tbody>
                                    <!-- Excluir Comprador -->
                                    <div class="modal fade" id="modalExcluirComprador<?php echo $compradorProposta['id_comprador']; ?>" tabindex="-1" aria-labelledby="emodalExcluirCompradorLabel<?php echo $compradorProposta['id_comprador']; ?>" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="emodalExcluirCompradorLabel<?php echo $compradorProposta['id_comprador']; ?>">Excluir Comprador</h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p style="color:red;text-align:center;"><b>você esta prestes a excluir a um comprador, gostaria de processeguir?</b></p>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                                <a href="deletComprador.php?id=<?php echo $compradorProposta['id_comprador'];?>">
                                                    <button type="button" class="btn btn-danger">Excluir</button>
                                                </a>
                                            </div>
                                            </div>
                                        </div>
                                    </div>
                                        <!-- Modal Doc Comprador -->
                                        <div class="modal fade" id="modalDocComp<?php echo $compradorProposta['id_comprador']; ?>" tabindex="-1" aria-labelledby="exmodalDocComp<?php echo $compradorProposta['id_comprador']; ?>" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                                <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="exmodalDocComp<?php echo $compradorProposta['id_comprador']; ?>">Documentos de <?php echo removerTextoDepoisDoEspaco($compradorProposta['nome_comprador']); ?></h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="docArea">
                                                        <?php 
                                                            $id_comprador = $compradorProposta['id_comprador'];
                                                            $sql_dadosDocComprador = "SELECT * FROM db_doc_comp WHERE id_comprador = $id_comprador";
                                                            $query_dadosDocComprador = $mysqli->query($sql_dadosDocComprador) or die($mysqli->error);
                                                            $num_dadosDocComprador = $query_dadosDocComprador->num_rows;
                                                            $dadosDocComprador = $query_dadosDocComprador->fetch_assoc();
                                                        
                                                        ?>
                                                        <div class="docuser">
                                                            <span>RG/CNH: </span>
                                                            <?php if($dadosDocComprador['path_rg_cnh'] != '' || $dadosDocComprador['path_rg_cnh'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_rg_cnh'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_rg_cnh">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>
                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>CPF: </span>
                                                            <?php if($dadosDocComprador['path_cpf'] != '' || $dadosDocComprador['path_cpf'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_cpf'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_cpf">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>

                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>Comprovante de Residencia: </span>
                                                            <?php if($dadosDocComprador['path_comp_resi'] != '' || $dadosDocComprador['path_comp_resi'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_comp_resi'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_comp_resi">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>


                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>Comprovante de Renda: </span>
                                                            <?php if($dadosDocComprador['path_comp_ren'] != '' || $dadosDocComprador['path_comp_ren'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_comp_ren'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_comp_ren">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>

                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>Comprovante de Estado Civil: </span>
                                                            <?php if($dadosDocComprador['path_Comp_estado'] != '' || $dadosDocComprador['path_Comp_estado'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_Comp_estado'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_Comp_estado">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>

                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>Imposto de Renda: </span>
                                                            <?php if($dadosDocComprador['path_imp_renda'] != '' || $dadosDocComprador['path_imp_renda'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_imp_renda'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_imp_renda">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>









                                                <?php if($compradorProposta['estado_civil__comprador'] == 'casado') { ?>
                                                    <h5>Cônjuge</h5>
                                                    <hr>
                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>RG/CNH Cônjuge: </span>
                                                            <?php if($dadosDocComprador['path_conjugue_rg_cnh'] != '' || $dadosDocComprador['path_conjugue_rg_cnh'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_conjugue_rg_cnh'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_conjugue_rg_cnh">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>

                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>CPF Cônjuge: </span>
                                                            <?php if($dadosDocComprador['path_cpf_conjugue'] != '' || $dadosDocComprador['path_cpf_conjugue'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_cpf_conjugue'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_cpf_conjugue">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>

                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>Comprovante de Renda Cônjuge: </span>
                                                            <?php if($dadosDocComprador['path_comp_ren_conjug'] != '' || $dadosDocComprador['path_comp_ren_conjug'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_comp_ren_conjug'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_comp_ren_conjug">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>

                                                    <div class="docArea">
                                                        <div class="docuser">
                                                            <span>Comprovante de Renda Cônjuge: </span>
                                                            <?php if($dadosDocComprador['path_imp_renda_conjug'] != '' || $dadosDocComprador['path_imp_renda_conjug'] != Null) { ?>
                                                                <p><a href="../../../<?php echo $dadosDocComprador['path_imp_renda_conjug'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                            <?php } else { ?>
                                                                <p style="color:red;"><b>Documento não enviado!</b></p>
                                                            <?php } ?>
                                                        </div>
                                                        <div class="deletArea">
                                                            <!--Botão Deletar-->
                                                            <a href="deletarDocComprador.php?id=<?php echo $dadosDocComprador['id_proposta'] ?>&doc=path_imp_renda_conjug">
                                                                <button type="submit" class="button" >
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                        <g clip-path="url(#clip0_35_24)">
                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                        </g>
                                                                        <defs>
                                                                        <clipPath id="clip0_35_24">
                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                        </clipPath>
                                                                        </defs>
                                                                    </svg>

                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                        <g clip-path="url(#clip0_35_22)">
                                                                            <path
                                                                                fill="black"
                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                            ></path>
                                                                        </g>
                                                                        <defs>
                                                                            <clipPath id="clip0_35_22">
                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                            </clipPath>
                                                                        </defs>
                                                                    </svg>
                                                                </button>
                                                            </a>
                                                        </div>
                                                    </div>
                                                <?php } ?>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                                </div>
                                                </div>
                                            </div>
                                        </div>


                                    <?php } } ?>
                                </table>
                                <div class="btnStatusProposta">
                                    <button data-bs-toggle="modal" data-bs-target="#addComprador"  style="margin-left:auto;" class="cta">
                                        <span class="hover-underline-animation">Add Comprador</span>
                                        <svg
                                            id="arrow-horizontal"
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="30"
                                            height="10"
                                            viewBox="0 0 46 16"
                                        >
                                            <path
                                            id="Path_10"
                                            data-name="Path 10"
                                            d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                            transform="translate(30)"
                                            ></path>
                                        </svg>
                                    </button>
                                </div>
                                <!-- Modal Add Comprador-->
                                <div class="modal fade" id="addComprador" tabindex="-1" aria-labelledby="addCompradorLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-xl modal-dialog-centered">
                                        <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="../criacao/newBuyer.php?id=<?php echo $id_Proposta; ?>" method="post">
                                                    <div class="headerSubDados">
                                                        <h1><b>Informações Comprador</b></h1>
                                                        <hr>
                                                    </div>  
                                                    <div class="infoArea row">
                                                        <div class="col-md-6 col-sm-12">
                                                            <label for="nameComprador" class="form-label">Nome</label>
                                                            <input required name="nameComprador" type="text" id="nameComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="nascimentoComprador" class="form-label">Nascimento</label>
                                                            <input required name="nascimentoComprador" type="date" id="nascimentoComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label class="form-label">Estado Civil</label>
                                                            <select onchange="mostrarEsconderConjugue(this)" name="estadoCivil" class="form-select" aria-label="Default select example">
                                                                <option value="N/A">Selecione o estado civil</option>
                                                                <option value="solteiro">Solteiro</option>
                                                                <option value="casado">Casado</option>
                                                                <option value="divorciado">Divorciado</option>
                                                                <option value="viuvo">Víuvo</option>
                                                            </select>                                                    
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-8 col-sm-12">
                                                            <label for="emailComprador" class="form-label">E-mail</label>
                                                            <input required name="emailComprador" type="email" id="emailComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="telefoneComprador" class="form-label">Telefone</label>
                                                            <input required name="telefoneComprador" type="text" id="telefoneComprador" class="form-control telefone">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="rgComprador" class="form-label">RG</label>
                                                            <input required name="rgComprador" type="text" id="rgComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="emissorComprador" class="form-label">Orgão Emissor</label>
                                                            <input required name="emissorComprador" type="text" id="emissorComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="dataEmissaoComprador" class="form-label">Data Emissor</label>
                                                            <input required name="dataEmissaoComprador" type="date" id="dataEmissaoComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="cpfComprador" class="form-label">CPF</label>
                                                            <input required name="cpfComprador" type="text" id="cpfComprador" class="form-control cpfMask">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-6 col-sm-12">
                                                            <label for="nomeMaeComprador" class="form-label">Nome da Mãe</label>
                                                            <input required name="nomeMaeComprador" type="tect" id="nomeMaeComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-6 col-sm-12">
                                                            <label for="nomepaiComprador" class="form-label">Nome do Pai</label>
                                                            <input name="nomepaiComprador" type="text" id="nomepaiComprador" class="form-control">
                                                        </div>
                                                    </div>


                                                    <!--Cônjuge-->
                                                    <div class="conjugueDados" id="conjugueDados">
                                                        <div class="headerSubDados">
                                                            <h1><b>Informações Cônjuge</b></h1>
                                                            <hr>
                                                        </div>

                                                        <div class="infoArea row">
                                                            <div class="col-md-8 col-sm-12">
                                                                <label for="nameConjugue" class="form-label">Nome</label>
                                                                <input name="nameConjugue" type="text" id="nameConjugue" class="form-control">
                                                            </div>

                                                            <div class="col-md-4 col-sm-12">
                                                                <label for="nascimentoConjugue" class="form-label">Nascimento</label>
                                                                <input name="nascimentoConjugue" type="date" id="nascimentoConjugue" class="form-control">
                                                            </div>

                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-8 col-sm-12">
                                                                <label for="emailConjugue" class="form-label">E-mail</label>
                                                                <input name="emailConjugue" type="email" id="emailConjugue" class="form-control">
                                                            </div>

                                                            <div class="col-md-4 col-sm-12">
                                                                <label for="telefoneConjugue" class="form-label">Telefone</label>
                                                                <input name="telefoneConjugue" type="text" id="telefoneConjugue" class="form-control telefone">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="rgConjugue" class="form-label">RG</label>
                                                                <input name="rgConjugue" type="text" id="rgConjugue" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="emissorConjugue" class="form-label">Orgão Emissor</label>
                                                                <input name="emissorConjugue" type="text" id="emissorConjugue" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="dataEmissaoConjugue" class="form-label">Data Emissor</label>
                                                                <input name="dataEmissaoConjugue" type="date" id="dataEmissaoConjugue" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="cpfConjugue" class="form-label">CPF</label>
                                                                <input name="cpfConjugue" type="text" id="cpfConjugue" class="form-control cpfMask">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nomeMaeConjugue" class="form-label">Nome da Mãe</label>
                                                                <input name="nomeMaeConjugue" type="tect" id="nomeMaeConjugue" class="form-control">
                                                            </div>

                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nomepaiConjugue" class="form-label">Nome do Pai</label>
                                                                <input name="nomepaiConjugue" type="text" id="nomepaiConjugue" class="form-control">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea form-check form-switch">
                                                            <input value="Sim" onchange="mostrarEsconderRendaConjugue()" class="form-check-input" type="checkbox" role="switch" id="compoerenda" name="compoerenda">
                                                            <label class="form-check-label" for="compoerenda">Compoe renda?</label>
                                                        </div>

                                                        <div class="compRendaConjugue" id="compRendaConjugue">
                                                            <div class="headerSubDados">
                                                                <h1><b>Profissão e Renda do Cônjuge</b></h1>
                                                                <hr>
                                                            </div>
                                                            <div class="infoArea row">
                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="profissaoConj" class="form-label">Profissão</label>
                                                                    <input name="profissaoConj" type="tect" id="profissaoConj" class="form-control">
                                                                </div>

                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="admissaoConj" class="form-label">Data de Admissão</label>
                                                                    <input name="admissaoConj" type="text" id="admissaoConj" class="form-control">
                                                                </div>

                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="pisConj" class="form-label">Pis</label>
                                                                    <input name="pisConj" type="tect" id="pisConj" class="form-control pisInput">
                                                                </div>

                                                                <div class="col-md-6 col-sm-12">
                                                                    <label for="empregadorConj" class="form-label">Empregador</label>
                                                                    <input name="empregadorConj" type="text" id="empregadorConj" class="form-control">
                                                                </div>


                                                                <div class="col-md-6 col-sm-12">
                                                                    <label for="cnpjEmpregConj" class="form-label">CNPJ do Empregador</label>
                                                                    <input name="cnpjEmpregConj" type="tect" id="cnpjEmpregConj" class="form-control cnpjMAsk">
                                                                </div>

                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="tipoRendaConjComprador" class="form-label">Tipo de Renda</label>
                                                                    <select  name="tipoRendaConjComprador" class="form-select" id="tipoRendaConjComprador" aria-label="Default select example">
                                                                        <option value="N/A">Selecione um Tipo de Renda</option>
                                                                        <option value="formal">Formal</option>
                                                                        <option value="informal">Informal</option>
                                                                    </select>  
                                                                </div>

                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="rendaBrutaConjComprador" class="form-label">Renda Bruta</label>
                                                                    <input name="rendaBrutaConjComprador" type="text" id="rendaBrutaConjComprador" class="form-control formatReal">
                                                                </div>

                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="rendaLiquidaConjComprador" class="form-label">Renda Líquida</label>
                                                                    <input  name="rendaLiquidaConjComprador" type="text" id="rendaLiquidaConjComprador" class="form-control formatReal">
                                                                </div>

                                                            
                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="bancoConju" class="form-label">Banco</label>
                                                                    <select name="bancoConju" class="form-select" id="bancoConju" aria-label="Default select example">
                                                                        <option value="N/A">Selecione um Banco</option>
                                                                        <option value="itau">Itau</option>
                                                                        <option value="bradesco">Bradesco</option>
                                                                        <option value="santander">Santander</option>
                                                                        <option value="caixa">Caixa</option>
                                                                        <option value="inter">Inter</option>
                                                                    </select>  
                                                                </div>

                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="agenciaConj" class="form-label">Agência</label>
                                                                    <input name="agenciaConj" type="text" id="agenciaConj" class="form-control">
                                                                </div>

                                                                <div class="col-md-4 col-sm-12">
                                                                    <label for="contaConj" class="form-label">Conta</label>
                                                                    <input name="contaConj" type="tect" id="contaConj" class="form-control">
                                                                </div>

                                                            </div>
                                                        </div>



                                                    </div>
                                                    

                                                    <!--Endereço -->
                                                    <div class="headerSubDados">
                                                        <h1 class="direita"><b>Endereço</b></h1>
                                                        <hr>
                                                    </div>  
                                                    <div class="infoArea row">
                                                        <div disabled class="col-md-4 col-sm-12">
                                                            <label for="cepInput2" class="form-label">CEP</label>
                                                            <input required name="cepuser2" type="tect" id="cepInput2" class="form-control cepInput">
                                                        </div>

                                                        <div disabled class="col-md-4 col-sm-12">
                                                            <label for="cidadeuser2" class="form-label">Cidade</label>
                                                            <input required name="cidadeuser2" type="text" id="cidadeuser2" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="estadouser2" class="form-label">Estado</label>
                                                            <input required name="estadouser2" type="text" id="estadouser2" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="ruauser2" class="form-label">Rua</label>
                                                            <input required name="ruauser2" type="tect" id="ruauser2" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="completComprador" class="form-label">Complemento</label>
                                                            <input name="completComprador" type="text" id="completComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="numerocasaComprador" class="form-label">Número</label>
                                                            <input required name="numerocasaComprador" type="text" id="numerocasaComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="bairroComprador" class="form-label">Bairro</label>
                                                            <input required name="bairrouser2" type="text" id="bairroComprador" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="headerSubDados">
                                                        <h1 class=""><b>Profissão e Renda Comprador</b></h1>
                                                        <hr>
                                                    </div>  
                                                    <div class="infoArea row">
                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="profissaoComprador" class="form-label">Profissão</label>
                                                            <input required name="profissaoComprador" type="tect" id="profissaoComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="pisComprador" class="form-label">PIS</label>
                                                            <input required name="pisComprador" type="text" id="pisComprador" class="form-control pisInput">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="dataAdmissãoComprador" class="form-label">Data Admissão</label>
                                                            <input required name="dataAdmissãoComprador" type="date" id="dataAdmissãoComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-8 col-sm-12">
                                                            <label for="empregadorComprador" class="form-label">Empregador</label>
                                                            <input required name="empregadorComprador" type="text" id="empregadorComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="cnpjEmpregadorComprador" class="form-label">CNPJ</label>
                                                            <input required name="cnpjEmpregadorComprador" type="text" id="cnpjEmpregadorComprador" class="form-control cnpjMAsk">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="tipoRendaComprador" class="form-label">Tipo de Renda</label>
                                                            <select required name="tipoRendaComprador" class="form-select" id="tipoRendaComprador" aria-label="Default select example">
                                                                <option value="N/A">Selecione um Tipo de Renda</option>
                                                                <option value="formal">Formal</option>
                                                                <option value="informal">Informal</option>
                                                            </select>  
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="rendaBrutaComprador" class="form-label">Renda Bruta</label>
                                                            <input required name="rendaBrutaComprador" type="text" id="rendaBrutaComprador" class="form-control formatReal">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="rendaLiquidaComprador" class="form-label">Renda Líquida</label>
                                                            <input required name="rendaLiquidaComprador" type="text" id="rendaLiquidaComprador" class="form-control formatReal">
                                                        </div>
                                                    </div>
        
                                                    <div class="headerSubDados">
                                                            <h1 class="direita"><b>Dados Bancarios</b></h1>
                                                        <hr>
                                                    </div>  
                                                    <div class="infoArea row">
                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="bancoComprador" class="form-label">Banco</label>
                                                            <select required name="bancoComprador" class="form-select" id="bancoComprador" aria-label="Default select example">
                                                                <option value="N/A">Selecione um Banco</option>
                                                                <option value="itau">Itau</option>
                                                                <option value="bradesco">Bradesco</option>
                                                                <option value="santander">Santander</option>
                                                                <option value="caixa">Caixa</option>
                                                                <option value="inter">Inter</option>
                                                            </select>  
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="agenciaComprador" class="form-label">Agencia</label>
                                                            <input required name="agenciaComprador" type="text" id="agenciaComprador" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="contaComprador" class="form-label">Conta Bancaria</label>
                                                            <input required name="contaComprador" type="text" id="contaComprador" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="btnStatusProposta">
                                                        <button type="submit"  style="margin-left:auto;" class="cta">
                                                            <span class="hover-underline-animation">Adicionar</span>
                                                            <svg
                                                                id="arrow-horizontal"
                                                                xmlns="http://www.w3.org/2000/svg"
                                                                width="30"
                                                                height="10"
                                                                viewBox="0 0 46 16"
                                                            >
                                                                <path
                                                                id="Path_10"
                                                                data-name="Path 10"
                                                                d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                                                transform="translate(30)"
                                                                ></path>
                                                            </svg>
                                                        </button>
                                                    </div>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        <!--Final Tabela Comprador-->


                        <!--Tabela Vendedor-->
                            <div class="vendedorArea">
                                <div class="">
                                    <h1><b>Dados Vendedor</b></h1>
                                    <hr>
                                </div> 
                                <table class="table-proposta table">
                                    <thead>
                                        <tr>
                                            <th >Nome</th>
                                            <th >CPF/CNPJ</th>
                                            <th >Telefone</th>
                                            <th>Tipo</th>
                                            <th></th>
                                        </tr>
                                    </thead>
                                        <?php if($num_dadosVendedor == 0) { ?>
                                                            <tr>
                                                                <td colspan="6" style="text-align: center;">Nem um vendedor encontrado!</td>
                                                            </tr>
                                        <?php } else { 
                                            while($vendedorProposta = $query_dadosVendedor->fetch_assoc()){
                                            ?>
                                    <tbody>
                                        <tr scope="row">
                                            <th class="">
                                                <div class="">
                                                    <span class="">
                                                        <?php 
                                                            if ($vendedorProposta['nome_vendedor'] == NULL || $vendedorProposta['nome_vendedor'] == "") {
                                                                echo removerTextoDepoisDoEspaco($vendedorProposta['nome_empresarial']); 
                                                            } else {
                                                                echo removerTextoDepoisDoEspaco($vendedorProposta['nome_vendedor']);
                                                            }; 
                                                        ?>
                                                    </span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class="">
                                                        <?php 
                                                            if ($vendedorProposta['cpf_vendedor'] == NULL || $vendedorProposta['cpf_vendedor'] == "") {
                                                                echo $vendedorProposta['cnpj_vendedor']; 
                                                            } else {
                                                                echo $vendedorProposta['cpf_vendedor'];
                                                            }; 
                                                        ?>
                                                    </span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class="">
                                                        <?php 
                                                            if ($vendedorProposta['telefone_vendedor'] == NULL || $vendedorProposta['telefone_vendedor'] == "") {
                                                                echo $vendedorProposta['telefone_cnpj']; 
                                                            } else {
                                                                echo $vendedorProposta['telefone_vendedor'];
                                                            }; 
                                                        ?>
                                                    </span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class=""><?php if($vendedorProposta['e_pj'] == "nao") {echo "Pessoa Física";} else {echo "Pessoa Jurídica";} ?></span>
                                                </div>
                                            </th>
                                            <th class=" edicaoProposta" style="text-align: right;">
                                                <button class="buttonEditar" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 20 20" height="20" fill="none" class="svg-icon">
                                                        <g stroke-width="1.5" stroke-linecap="round" stroke="#4e4e4e">
                                                            <circle r="2.5" cy="10" cx="10"></circle>
                                                            <path fill-rule="evenodd" d="m8.39079 2.80235c.53842-1.51424 2.67991-1.51424 3.21831-.00001.3392.95358 1.4284 1.40477 2.3425.97027 1.4514-.68995 2.9657.82427 2.2758 2.27575-.4345.91407.0166 2.00334.9702 2.34248 1.5143.53842 1.5143 2.67996 0 3.21836-.9536.3391-1.4047 1.4284-.9702 2.3425.6899 1.4514-.8244 2.9656-2.2758 2.2757-.9141-.4345-2.0033.0167-2.3425.9703-.5384 1.5142-2.67989 1.5142-3.21831 0-.33914-.9536-1.4284-1.4048-2.34247-.9703-1.45148.6899-2.96571-.8243-2.27575-2.2757.43449-.9141-.01669-2.0034-.97028-2.3425-1.51422-.5384-1.51422-2.67994.00001-3.21836.95358-.33914 1.40476-1.42841.97027-2.34248-.68996-1.45148.82427-2.9657 2.27575-2.27575.91407.4345 2.00333-.01669 2.34247-.97026z" clip-rule="evenodd">
                                                            </path>
                                                        </g>
                                                    </svg>
                                                </button>
                                                <ul class="dropdown-menu">
                                                    <li><a class="dropdown-item" href="editarVendedor.php?id=<?php echo $vendedorProposta['id_vendedor']; ?>">Editar Vendedor</a></li>
                                                    <?php if($usuarioPrimario['permissao_usuario'] == 3 || $usuarioPrimario['permissao_usuario'] == 2) {?> <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalDocVend<?php echo $vendedorProposta['id_vendedor']; ?>">Documentos Vendedor</a></li> <?php } else {} ?>
                                                    <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalExcluirVendedor<?php echo $vendedorProposta['id_vendedor']; ?>">Excluir Vendedor</a></li>
                                                </ul>
                                            </th>
                                        </tr>
                                    </tbody>

                                    <!-- Excluir Vendedor -->
                                    <div class="modal fade" id="modalExcluirVendedor<?php echo $vendedorProposta['id_vendedor']; ?>" tabindex="-1" aria-labelledby="modalExcluirVendedorLabel<?php echo $vendedorProposta['id_vendedor']; ?>" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="modalExcluirVendedorLabel<?php echo $vendedorProposta['id_vendedor'] ?>">Excluir Vendedor</h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p style="color:red;text-align:center;"><b>você esta prestes a excluir a um vendedor, gostaria de processeguir?</b></p>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                                <a href="deletVendedor.php?id=<?php echo $vendedorProposta['id_vendedor'];?>">
                                                    <button type="button" class="btn btn-danger">Excluir</button>
                                                </a>
                                            </div>
                                            </div>
                                        </div>
                                    </div>

                                        <!-- Modal Doc Comprador -->
                                        <div class="modal fade" id="modalDocVend<?php echo $vendedorProposta['id_vendedor']; ?>" tabindex="-1" aria-labelledby="exmodalDocVend<?php echo $vendedorProposta['id_vendedor']; ?>" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                                <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="exmodalDocVend<?php echo $vendedorProposta['id_vendedor']; ?>">Documentos de <?php if($vendedorProposta['e_pj'] == "nao" ) {echo removerTextoDepoisDoEspaco($vendedorProposta['nome_vendedor']);} else {echo removerTextoDepoisDoEspaco($vendedorProposta['nome_empresarial']);} ?></h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <?php
                                                        $id_vendedor = $vendedorProposta['id_vendedor'];
                                                        $sql_dadosDocVendedor = "SELECT * FROM db_doc_vend WHERE id_vendedor = $id_vendedor";
                                                        $query_dadosDocVendedor = $mysqli->query($sql_dadosDocVendedor) or die($mysqli->error);
                                                        $num_dadosDocVendedor = $query_dadosDocVendedor->num_rows;
                                                        $dadosDocVendedor = $query_dadosDocVendedor->fetch_assoc();

                                                        if($vendedorProposta['e_pj'] == 'nao') {
                                                    ?>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>RG/CNH: </span>
                                                                    <?php if(isset($dadosDocVendedor['path_rg_cnh']) && $dadosDocVendedor['path_rg_cnh'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocVendedor['path_rg_cnh'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=path_rg_cnh" <?php } else {}?>>
                                                                        <button type="submit" class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>CPF: </span>
                                                                    <?php if(isset($dadosDocVendedor['path_cpf']) && $dadosDocVendedor['path_cpf'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocVendedor['path_cpf'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=path_cpf" <?php } else {}?>>

                                                                        <button type="submit" class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>Comprovante de Residencia: </span>
                                                                    <?php if(isset($dadosDocVendedor['path_comp_resi']) && $dadosDocVendedor['path_comp_resi'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocVendedor['path_comp_resi'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=path_comp_resi" <?php } else {}?>>

                                                                    
                                                                        <button type="submit" class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>Comprovante de Estado Civil: </span>
                                                                    <?php if(isset($dadosDocVendedor['path_Comp_estado']) && $dadosDocVendedor['path_Comp_estado'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocVendedor['path_Comp_estado'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=path_Comp_estado" <?php } else {}?>>

                                                                        <button type="submit" class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>

                                                            <?php if($vendedorProposta['estado_civil_vendedor'] == 'casado') { ?>
                                                                <h5>Cônjuge</h5>
                                                                <hr>
                                                                <div class="docArea">
                                                                    <div class="docuser">
                                                                        <span>RG/CNH Cônjuge: </span>
                                                                        <?php if(isset($dadosDocVendedor['path_conjugue_rg_cnh']) && $dadosDocVendedor['path_conjugue_rg_cnh'] != '') { ?>
                                                                            <p><a href="../../../<?php echo $dadosDocVendedor['path_conjugue_rg_cnh'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                        <?php } else { ?>
                                                                            <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                        <?php } ?>
                                                                    </div>
                                                                    <div class="deletArea">
                                                                        <!--Botão Deletar-->
                                                                        <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=path_conjugue_rg_cnh" <?php } else {}?>>

                                                                            <button type="submit" class="button" >
                                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                    <g clip-path="url(#clip0_35_24)">
                                                                                        <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                    </g>
                                                                                    <defs>
                                                                                    <clipPath id="clip0_35_24">
                                                                                        <rect fill="white" height="14" width="69"></rect>
                                                                                    </clipPath>
                                                                                    </defs>
                                                                                </svg>

                                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                    <g clip-path="url(#clip0_35_22)">
                                                                                        <path
                                                                                            fill="black"
                                                                                            d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                        ></path>
                                                                                    </g>
                                                                                    <defs>
                                                                                        <clipPath id="clip0_35_22">
                                                                                            <rect fill="white" height="57" width="69"></rect>
                                                                                        </clipPath>
                                                                                    </defs>
                                                                                </svg>
                                                                            </button>
                                                                        </a>
                                                                    </div>
                                                                </div>
                                                                <div class="docArea">
                                                                    <div class="docuser">
                                                                        <span>CPF Cônjuge: </span>
                                                                        <?php if(isset($dadosDocVendedor['path_cpf_conjugue']) && $dadosDocVendedor['path_cpf_conjugue'] != '') { ?>
                                                                            <p><a href="../../../<?php echo $dadosDocVendedor['path_cpf_conjugue'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                        <?php } else { ?>
                                                                            <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                        <?php } ?>
                                                                    </div>
                                                                    <div class="deletArea">
                                                                        <!--Botão Deletar-->
                                                                            <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=path_cpf_conjugue" <?php } else {}?>>

                                                                            
                                                                                <button type="submit" class="button" >
                                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                        <g clip-path="url(#clip0_35_24)">
                                                                                            <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                        </g>
                                                                                        <defs>
                                                                                        <clipPath id="clip0_35_24">
                                                                                            <rect fill="white" height="14" width="69"></rect>
                                                                                        </clipPath>
                                                                                        </defs>
                                                                                    </svg>

                                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                        <g clip-path="url(#clip0_35_22)">
                                                                                            <path
                                                                                                fill="black"
                                                                                                d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                            ></path>
                                                                                        </g>
                                                                                        <defs>
                                                                                            <clipPath id="clip0_35_22">
                                                                                                <rect fill="white" height="57" width="69"></rect>
                                                                                            </clipPath>
                                                                                        </defs>
                                                                                    </svg>
                                                                                </button>
                                                                            </a>
                                                                    </div>
                                                                </div>
                                                            <?php } ?>
                                                        <?php } else { ?>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>Contrato Social: </span>
                                                                    <?php if(isset($dadosDocVendedor['contrato_social']) && $dadosDocVendedor['contrato_social'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocVendedor['contrato_social'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=contrato_social" <?php } else {}?>>
                                                                        <button class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>RG/CNH Representante do CNPJ: </span>
                                                                    <?php if(isset($dadosDocVendedor['rg_cnh_repre']) && $dadosDocVendedor['rg_cnh_repre'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocVendedor['rg_cnh_repre'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=rg_cnh_repre" <?php } else {}?>>
                                                                        <button class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>CPF Representante do CNPJ: </span>
                                                                    <?php if(isset($dadosDocVendedor['cpf_repre']) && $dadosDocVendedor['cpf_repre'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocVendedor['cpf_repre'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocVendedor['id_proposta'])) { ?> href="deletarDocVendedor.php?id=<?php  echo $dadosDocVendedor['id_proposta'] ?>&doc=cpf_repre" <?php } else {}?>>
                                                                        <button class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                        
                                                        <?php } ?>

                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                                </div>
                                                </div>
                                            </div>
                                        </div>



                                    <?php } } ?>
                                </table>
                                <div class="btnStatusProposta">
                                    <button data-bs-toggle="modal" data-bs-target="#addVendedor"  style="margin-left:auto;" class="cta">
                                        <span class="hover-underline-animation">Add Vendedor</span>
                                        <svg
                                            id="arrow-horizontal"
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="30"
                                            height="10"
                                            viewBox="0 0 46 16"
                                        >
                                            <path
                                            id="Path_10"
                                            data-name="Path 10"
                                            d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                            transform="translate(30)"
                                            ></path>
                                        </svg>
                                    </button>
                                </div>
                                <!-- Modal Add Vendedor-->
                                <div class="modal fade" id="addVendedor" tabindex="-1" aria-labelledby="addVendedorLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-xl modal-dialog-centered">
                                        <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="../criacao/newSalesperson.php?id=<?php echo $id_Proposta; ?>" method="post"> 
                                                    <div class="infoArea form-check form-switch">
                                                        <input value="Sim" onchange="toggleDivs()" class="form-check-input" type="checkbox" role="switch" id="epf" name="epf">
                                                        <label class="form-check-label" for="epf">Pessoa Jurídica?</label>
                                                    </div>
                                                    <div class="vendedorFisico">
                                                        <div class="headerSubDados">
                                                            <h1><b>Informações Vendedor PF</b></h1>
                                                            <hr>
                                                        </div> 
                                                        <div class="infoArea row">
                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nameVendedor" class="form-label">Nome</label>
                                                                <input required name="nameVendedor" type="text" id="nameVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="nascimentoVendedor" class="form-label">Nascimento</label>
                                                                <input required name="nascimentoVendedor" type="date" id="nascimentoVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label class="form-label">Estado Civil</label>
                                                                <select onchange="mostrarEsconderConjugueVendedor(this)" name="estadoCivilVendedor" class="form-select" aria-label="Default select example">
                                                                    <option value="N/A">Selecione o estado civil</option>
                                                                    <option value="solteiro">Solteiro</option>
                                                                    <option value="casado">Casado</option>
                                                                    <option value="divorciado">Divorciado</option>
                                                                    <option value="viuvo">Víuvo</option>
                                                                </select>                                                    
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-8 col-sm-12">
                                                                <label for="emailVendedor" class="form-label">E-mail</label>
                                                                <input required name="emailVendedor" type="email" id="emailVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-4 col-sm-12">
                                                                <label for="telefoneVendedor" class="form-label">Telefone</label>
                                                                <input required name="telefoneVendedor" type="text" id="telefoneVendedor" class="form-control telefone">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="rgVendedor" class="form-label">RG</label>
                                                                <input required name="rgVendedor" type="text" id="rgVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="emissorVendedor" class="form-label">Orgão Emissor</label>
                                                                <input required name="emissorVendedor" type="text" id="emissorVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="dataEmissaoVendedor" class="form-label">Data Emissor</label>
                                                                <input required name="dataEmissaoVendedor" type="date" id="dataEmissaoVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="cpfVendedor" class="form-label">CPF</label>
                                                                <input required name="cpfVendedor" type="text" id="cpfVendedor" class="form-control cpfMask">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nomeMaeVendedor" class="form-label">Nome da Mãe</label>
                                                                <input required name="nomeMaeVendedor" type="tect" id="nomeMaeVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nomepaiVendedor" class="form-label">Nome do Pai</label>
                                                                <input name="nomepaiVendedor" type="text" id="nomepaiVendedor" class="form-control">
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <div class="vendedorJuridico">
                                                        <div class="headerSubDados">
                                                            <h1><b>Informações Vendedor PJ</b></h1>
                                                            <hr>
                                                        </div> 
                                                        <div class="infoArea row">
                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nomeVendedorPj" class="form-label">Razão Social</label>
                                                                <input name="nomeVendedorPj" type="tect" id="nomeVendedorPj" class="form-control">
                                                            </div>

                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="cnpjVendedorPJ" class="form-label">CNPJ</label>
                                                                <input name="cnpjVendedorPJ" type="text" id="cnpjVendedorPJ" class="cnpjMAsk form-control">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="emailVendedorPJ" class="form-label">E-mail</label>
                                                                <input name="emailVendedorPJ" type="tect" id="emailVendedorPJ" class="form-control">
                                                            </div>

                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="telefoneVendedorPj" class="form-label">Telefone</label>
                                                                <input name="telefoneVendedorPj" type="text" id="telefoneVendedorPj" class="telefone form-control">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="representantePj" class="form-label">Nome do Representante do CNPJ</label>
                                                                <input name="representantePj" type="tect" id="representantePj" class="form-control">
                                                            </div>

                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="cpjRepresentanteCNPJ" class="form-label">CPF do Representante do CNPJ</label>
                                                                <input name="cpjRepresentanteCNPJ" type="text" id="cpjRepresentanteCNPJ" class="cpfMask form-control">
                                                            </div>
                                                        </div>
                                                    </div>


                                                    <!--Cônjuge-->
                                                    <div class="conjugueDados" id="conjugueDadosvendedor">
                                                        <div class="headerSubDados">
                                                            <h1><b>Informações Cônjuge</b></h1>
                                                            <hr>
                                                        </div>

                                                        <div class="infoArea row">
                                                            <div class="col-md-8 col-sm-12">
                                                                <label for="nameConjugueVendedor" class="form-label">Nome</label>
                                                                <input name="nameConjugueVendedor" type="text" id="nameConjugueVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-4 col-sm-12">
                                                                <label for="nascimentoConjugueVendedor" class="form-label">Nascimento</label>
                                                                <input name="nascimentoConjugueVendedor" type="date" id="nascimentoConjugueVendedor" class="form-control">
                                                            </div>

                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-8 col-sm-12">
                                                                <label for="emailConjugueVendedor" class="form-label">E-mail</label>
                                                                <input name="emailConjugueVendedor" type="email" id="emailConjugueVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-4 col-sm-12">
                                                                <label for="telefoneConjugueVendedor" class="form-label">Telefone</label>
                                                                <input name="telefoneConjugueVendedor" type="text" id="telefoneConjugueVendedor" class="form-control telefone">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="rgConjugueVendedor" class="form-label">RG</label>
                                                                <input name="rgConjugueVendedor" type="text" id="rgConjugueVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="emissorConjugueVendedor" class="form-label">Orgão Emissor</label>
                                                                <input name="emissorConjugueVendedor" type="text" id="emissorConjugueVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="dataEmissaoConjugueVendedor" class="form-label">Data Emissor</label>
                                                                <input name="dataEmissaoConjugueVendedor" type="date" id="dataEmissaoConjugueVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-3 col-sm-12">
                                                                <label for="cpfConjugueVendedor" class="form-label">CPF</label>
                                                                <input name="cpfConjugueVendedor" type="text" id="cpfConjugueVendedor" class="form-control cpfMask">
                                                            </div>
                                                        </div>
                                                        <div class="infoArea row">
                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nomeMaeConjugueVendedor" class="form-label">Nome da Mãe</label>
                                                                <input name="nomeMaeConjugueVendedor" type="tect" id="nomeMaeConjugueVendedor" class="form-control">
                                                            </div>

                                                            <div class="col-md-6 col-sm-12">
                                                                <label for="nomepaiConjugueVendedor" class="form-label">Nome do Pai</label>
                                                                <input name="nomepaiConjugueVendedor" type="text" id="nomepaiConjugueVendedor" class="form-control">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    

                                                    <!--Endereço -->
                                                    <div class="headerSubDados">
                                                        <h1 class="direita"><b>Endereço</b></h1>
                                                        <hr>
                                                    </div>  
                                                    <div class="infoArea row">
                                                        <div disabled class="col-md-4 col-sm-12">
                                                            <label for="cepInput3" class="form-label">CEP</label>
                                                            <input required name="cepuser3" type="tect" id="cepInput3" class="form-control cepInput">
                                                        </div>

                                                        <div disabled class="col-md-4 col-sm-12">
                                                            <label for="cidadeuser3" class="form-label">Cidade</label>
                                                            <input required name="cidadeuser3" type="text" id="cidadeuser3" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="estadouser3" class="form-label">Estado</label>
                                                            <input required name="estadouser3" type="text" id="estadouser3" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="ruauser3" class="form-label">Rua</label>
                                                            <input required name="ruauser3" type="tect" id="ruauser3" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="completVendedor" class="form-label">Complemento</label>
                                                            <input name="completVendedor" type="text" id="completVendedor" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="numerocasaVendedor" class="form-label">Número</label>
                                                            <input required name="numerocasaVendedor" type="text" id="numerocasaVendedor" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="bairrouser3" class="form-label">Bairro</label>
                                                            <input required name="bairrouser3" type="text" id="bairrouser3" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="profissaoVendedorPF">
                                                        <div class="headerSubDados">
                                                            <h1 class=""><b>Profissão</b></h1>
                                                            <hr>
                                                        </div>  
                                                        <div class="infoArea row">
                                                            <div class="col-md-12">
                                                                <label for="profissaoVendedor" class="form-label">Profissão</label>
                                                                <input required name="profissaoVendedor" type="tect" id="profissaoVendedor" class="form-control">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="headerSubDados">
                                                            <h1 class="direita"><b>Dados Bancarios</b></h1>
                                                        <hr>
                                                    </div>  
                                                    <div class="infoArea row">
                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="bancoVendedor" class="form-label">Banco</label>
                                                            <select required name="bancoVendedor" class="form-select" id="bancoVendedor" aria-label="Default select example">
                                                                <option value="N/A">Selecione um Banco</option>
                                                                <option value="itau">Itau</option>
                                                                <option value="bradesco">Bradesco</option>
                                                                <option value="santander">Santander</option>
                                                                <option value="caixa">Caixa</option>
                                                                <option value="inter">Inter</option>
                                                            </select>  
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="agenciaVendedor" class="form-label">Agencia</label>
                                                            <input required name="agenciaVendedor" type="text" id="agenciaVendedor" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="contaVendedor" class="form-label">Conta Bancaria</label>
                                                            <input required name="contaVendedor" type="text" id="contaVendedor" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="btnStatusProposta">
                                                        <button type="submit"  style="margin-left:auto;" class="cta">
                                                            <span class="hover-underline-animation">Adicionar</span>
                                                            <svg
                                                                id="arrow-horizontal"
                                                                xmlns="http://www.w3.org/2000/svg"
                                                                width="30"
                                                                height="10"
                                                                viewBox="0 0 46 16"
                                                            >
                                                                <path
                                                                id="Path_10"
                                                                data-name="Path 10"
                                                                d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                                                transform="translate(30)"
                                                                ></path>
                                                            </svg>
                                                        </button>
                                                    </div>
                                            </form>
                                        </div>
                                    </div>
                                </div>



                            </div>
                        <!--Final Tabela Vendedor-->
                        <!--Tabela Procurador-->
                            <div class="vendedorArea">
                                <div class="">
                                    <h1><b>Dados Procurador</b></h1>
                                    <hr>
                                </div> 
                                <table class="table-proposta table">
                                    <thead>
                                        <tr>
                                            <th >Nome</th>
                                            <th >CPF</th>
                                            <th >Telefone</th>
                                            <th>Estado Civil</th>
                                            <th></th>
                                        </tr>
                                    </thead>
                                        <?php if($num_dadosProcurador == 0) { ?>
                                                            <tr>
                                                                <td colspan="6" style="text-align: center;">Nem um procurador encontrado!</td>
                                                            </tr>
                                        <?php } else { 
                                            while($procuradorProposta = $query_dadosProcurador->fetch_assoc()){
                                            ?>
                                    <tbody>
                                        <tr scope="row">
                                            <th class="">
                                                <div class="">
                                                    <span class="">
                                                        <?php echo removerTextoDepoisDoEspaco($procuradorProposta['nome_procurador']);?>
                                                    </span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class="">
                                                        <?php 
                                                                echo $procuradorProposta['cpf_procurador'];
                                                        ?>
                                                    </span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class="">
                                                        <?php 
                                                                echo $procuradorProposta['telefone_procurador'];
                                                        ?>
                                                    </span>
                                                </div>
                                            </th>
                                            <th class="">
                                                <div class="">
                                                    <span class=""><?php  echo $procuradorProposta['estado_civil_procurador'];  ?></span>
                                                </div>
                                            </th>
                                            <th class=" edicaoProposta" style="text-align: right;">
                                                <button class="buttonEditar" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 20 20" height="20" fill="none" class="svg-icon">
                                                        <g stroke-width="1.5" stroke-linecap="round" stroke="#4e4e4e">
                                                            <circle r="2.5" cy="10" cx="10"></circle>
                                                            <path fill-rule="evenodd" d="m8.39079 2.80235c.53842-1.51424 2.67991-1.51424 3.21831-.00001.3392.95358 1.4284 1.40477 2.3425.97027 1.4514-.68995 2.9657.82427 2.2758 2.27575-.4345.91407.0166 2.00334.9702 2.34248 1.5143.53842 1.5143 2.67996 0 3.21836-.9536.3391-1.4047 1.4284-.9702 2.3425.6899 1.4514-.8244 2.9656-2.2758 2.2757-.9141-.4345-2.0033.0167-2.3425.9703-.5384 1.5142-2.67989 1.5142-3.21831 0-.33914-.9536-1.4284-1.4048-2.34247-.9703-1.45148.6899-2.96571-.8243-2.27575-2.2757.43449-.9141-.01669-2.0034-.97028-2.3425-1.51422-.5384-1.51422-2.67994.00001-3.21836.95358-.33914 1.40476-1.42841.97027-2.34248-.68996-1.45148.82427-2.9657 2.27575-2.27575.91407.4345 2.00333-.01669 2.34247-.97026z" clip-rule="evenodd">
                                                            </path>
                                                        </g>
                                                    </svg>
                                                </button>
                                                <ul class="dropdown-menu">
                                                    <li><a class="dropdown-item" href="editarProcurador.php?id=<?php echo $procuradorProposta['id_procurador']; ?>">Editar Procurador</a></li>
                                                    <?php if($usuarioPrimario['permissao_usuario'] == 3 || $usuarioPrimario['permissao_usuario'] == 2) {?> <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalDocProc<?php echo $procuradorProposta['id_procurador']; ?>">Documentos Vendedor</a></li> <?php } else {} ?>
                                                    <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalExcluirProcurador<?php echo $procuradorProposta['id_procurador']; ?>">Excluir Procurador</a></li>
                                                </ul>
                                            </th>
                                        </tr>
                                    </tbody>

                                    <!-- Excluir Procurador -->
                                    <div class="modal fade" id="modalExcluirProcurador<?php echo $procuradorProposta['id_procurador']; ?>" tabindex="-1" aria-labelledby="modalExcluirProcuradorLabel<?php echo $procuradorProposta['id_procurador']; ?>" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="modalExcluirProcuradorLabel<?php $procuradorProposta['id_procurador']; ?>">Excluir Procurador</h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p style="color:red;text-align:center;"><b>você esta prestes a excluir a um Procurador, gostaria de processeguir?</b></p>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                                <a href="deletProcurador.php?id=<?php echo $procuradorProposta['id_procurador'];?>">
                                                    <button type="button" class="btn btn-danger">Excluir</button>
                                                </a>
                                            </div>
                                            </div>
                                        </div>
                                    </div>

                                        <!-- Modal Doc Procurador -->
                                        <div class="modal fade" id="modalDocProc<?php echo $procuradorProposta['id_procurador']; ?>" tabindex="-1" aria-labelledby="exmodalDocProc<?php echo $procuradorProposta['id_procurador']; ?>" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                                <div class="modal-content">
                                                <div class="modal-header">
                                                    <h1 class="modal-title fs-5" id="exmodalDocProc<?php echo $procuradorProposta['id_procurador']; ?>">Documentos de <?php echo removerTextoDepoisDoEspaco($procuradorProposta['nome_procurador']); ?></h1>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <?php
                                                        $id_procurador = $procuradorProposta['id_procurador'];
                                                        $sql_dadosDocProcurador = "SELECT * FROM db_doc_proc WHERE id_procurador = $id_procurador";
                                                        $query_dadosDocProcurador = $mysqli->query($sql_dadosDocProcurador) or die($mysqli->error);
                                                        $num_dadosDocProcurador = $query_dadosDocProcurador->num_rows;
                                                        $dadosDocProcurador = $query_dadosDocProcurador->fetch_assoc();
                                                    ?>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>RG/CNH: </span>
                                                                    <?php if(isset($dadosDocProcurador['path_rg_cnh']) && $dadosDocProcurador['path_rg_cnh'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocProcurador['path_rg_cnh'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocProcurador['id_proposta'])) { ?> href="deletarDocProcurador.php?id=<?php  echo $dadosDocProcurador['id_proposta'] ?>&doc=path_rg_cnh" <?php } else {}?>>
                                                                        <button type="submit" class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>CPF: </span>
                                                                    <?php if(isset($dadosDocProcurador['path_cpf']) && $dadosDocProcurador['path_cpf'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocProcurador['path_cpf'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocProcurador['id_proposta'])) { ?> href="deletarDocProcurador.php?id=<?php  echo $dadosDocProcurador['id_proposta'] ?>&doc=path_cpf" <?php } else {}?>>

                                                                        <button type="submit" class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            <div class="docArea">
                                                                <div class="docuser">
                                                                    <span>Procuração Vigente: </span>
                                                                    <?php if(isset($dadosDocProcurador['path_procu']) && $dadosDocProcurador['path_procu'] != '') { ?>
                                                                        <p><a href="../../../<?php echo $dadosDocProcurador['path_procu'] ?>" target="_blank"><b>Click Aqui</b></a></p>
                                                                    <?php } else { ?>
                                                                        <p style="color:red;"><b>Documento não enviado!</b></p>
                                                                    <?php } ?>
                                                                </div>
                                                                <div class="deletArea">
                                                                    <!--Botão Deletar-->
                                                                    <a <?php if(isset($dadosDocProcurador['id_proposta'])) { ?> href="deletarDocProcurador.php?id=<?php  echo $dadosDocProcurador['id_proposta'] ?>&doc=path_procu" <?php } else {}?>>

                                                                    
                                                                        <button type="submit" class="button" >
                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 14" class="svgIcon bin-top">
                                                                                <g clip-path="url(#clip0_35_24)">
                                                                                    <path fill="black" d="M20.8232 2.62734L19.9948 4.21304C19.8224 4.54309 19.4808 4.75 19.1085 4.75H4.92857C2.20246 4.75 0 6.87266 0 9.5C0 12.1273 2.20246 14.25 4.92857 14.25H64.0714C66.7975 14.25 69 12.1273 69 9.5C69 6.87266 66.7975 4.75 64.0714 4.75H49.8915C49.5192 4.75 49.1776 4.54309 49.0052 4.21305L48.1768 2.62734C47.3451 1.00938 45.6355 0 43.7719 0H25.2281C23.3645 0 21.6549 1.00938 20.8232 2.62734ZM64.0023 20.0648C64.0397 19.4882 63.5822 19 63.0044 19H5.99556C5.4178 19 4.96025 19.4882 4.99766 20.0648L8.19375 69.3203C8.44018 73.0758 11.6746 76 15.5712 76H53.4288C57.3254 76 60.5598 73.0758 60.8062 69.3203L64.0023 20.0648Z"></path>
                                                                                </g>
                                                                                <defs>
                                                                                <clipPath id="clip0_35_24">
                                                                                    <rect fill="white" height="14" width="69"></rect>
                                                                                </clipPath>
                                                                                </defs>
                                                                            </svg>

                                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 69 57" class="svgIcon bin-bottom">
                                                                                <g clip-path="url(#clip0_35_22)">
                                                                                    <path
                                                                                        fill="black"
                                                                                        d="M20.8232 -16.3727L19.9948 -14.787C19.8224 -14.4569 19.4808 -14.25 19.1085 -14.25H4.92857C2.20246 -14.25 0 -12.1273 0 -9.5C0 -6.8727 2.20246 -4.75 4.92857 -4.75H64.0714C66.7975 -4.75 69 -6.8727 69 -9.5C69 -12.1273 66.7975 -14.25 64.0714 -14.25H49.8915C49.5192 -14.25 49.1776 -14.4569 49.0052 -14.787L48.1768 -16.3727C47.3451 -17.9906 45.6355 -19 43.7719 -19H25.2281C23.3645 -19 21.6549 -17.9906 20.8232 -16.3727ZM64.0023 1.0648C64.0397 0.4882 63.5822 0 63.0044 0H5.99556C5.4178 0 4.96025 0.4882 4.99766 1.0648L8.19375 50.3203C8.44018 54.0758 11.6746 57 15.5712 57H53.4288C57.3254 57 60.5598 54.0758 60.8062 50.3203L64.0023 1.0648Z"
                                                                                    ></path>
                                                                                </g>
                                                                                <defs>
                                                                                    <clipPath id="clip0_35_22">
                                                                                        <rect fill="white" height="57" width="69"></rect>
                                                                                    </clipPath>
                                                                                </defs>
                                                                            </svg>
                                                                        </button>
                                                                    </a>
                                                                </div>
                                                            </div>
                                                            

                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                                </div>
                                                </div>
                                            </div>
                                        </div>



                                    <?php } } ?>
                                </table>
                                <div class="btnStatusProposta">
                                    <button data-bs-toggle="modal" data-bs-target="#addProcurador"  style="margin-left:auto;" class="cta">
                                        <span class="hover-underline-animation">Add Procurador</span>
                                        <svg
                                            id="arrow-horizontal"
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="30"
                                            height="10"
                                            viewBox="0 0 46 16"
                                        >
                                            <path
                                            id="Path_10"
                                            data-name="Path 10"
                                            d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                            transform="translate(30)"
                                            ></path>
                                        </svg>
                                    </button>
                                </div>

                                <!-- Modal Add Procurador-->
                                <div class="modal fade" id="addProcurador" tabindex="-1" aria-labelledby="addProcuradorLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-xl modal-dialog-centered">
                                        <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form action="../criacao/newAttorney.php?id=<?php echo $id_Proposta; ?>" method="post"> 
                                                    <div class="headerSubDados">
                                                        <h1><b>Informações Procurador</b></h1>
                                                        <hr>
                                                    </div> 
                                                    <div class="infoArea row">
                                                        <div class="col-md-6 col-sm-12">
                                                            <label for="nameProcurador" class="form-label">Nome</label>
                                                            <input required name="nameProcurador" type="text" id="nameProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="nascimentoProcurador" class="form-label">Nascimento</label>
                                                            <input required name="nascimentoProcurador" type="date" id="nascimentoProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label class="form-label">Estado Civil</label>
                                                            <select name="estadoCivilProcurador" class="form-select" aria-label="Default select example">
                                                                <option value="N/A">Selecione o estado civil</option>
                                                                <option value="solteiro">Solteiro</option>
                                                                <option value="casado">Casado</option>
                                                                <option value="divorciado">Divorciado</option>
                                                                <option value="viuvo">Víuvo</option>
                                                            </select>                                                    
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-8 col-sm-12">
                                                            <label for="emailProcurador" class="form-label">E-mail</label>
                                                            <input required name="emailProcurador" type="email" id="emailProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="telefoneProcurador" class="form-label">Telefone</label>
                                                            <input required name="telefoneProcurador" type="text" id="telefoneProcurador" class="form-control telefone">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="rgProcurador" class="form-label">RG</label>
                                                            <input required name="rgProcurador" type="text" id="rgProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="emissorProcurador" class="form-label">Orgão Emissor</label>
                                                            <input required name="emissorProcurador" type="text" id="emissorProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="dataEmissaoProcurador" class="form-label">Data Emissor</label>
                                                            <input required name="dataEmissaoProcurador" type="date" id="dataEmissaoProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="cpfProcurador" class="form-label">CPF</label>
                                                            <input required name="cpfProcurador" type="text" id="cpfProcurador" class="form-control cpfMask">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-6 col-sm-12">
                                                            <label for="nomeMaeProcurador" class="form-label">Nome da Mãe</label>
                                                            <input required name="nomeMaeProcurador" type="tect" id="nomeMaeProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-6 col-sm-12">
                                                            <label for="nomepaiProcurador" class="form-label">Nome do Pai</label>
                                                            <input name="nomepaiProcurador" type="text" id="nomepaiProcurador" class="form-control">
                                                        </div>
                                                    </div>

                                                    <!--Endereço -->
                                                    <div class="headerSubDados">
                                                        <h1 class="direita"><b>Endereço</b></h1>
                                                        <hr>
                                                    </div>  
                                                    <div class="infoArea row">
                                                        <div disabled class="col-md-4 col-sm-12">
                                                            <label for="cepInput4" class="form-label">CEP</label>
                                                            <input required name="cepInput4" type="tect" id="cepInput4" class="form-control cepInput">
                                                        </div>

                                                        <div disabled class="col-md-4 col-sm-12">
                                                            <label for="cidadeuser4" class="form-label">Cidade</label>
                                                            <input required name="cidadeuser4" type="text" id="cidadeuser4" class="form-control">
                                                        </div>

                                                        <div class="col-md-4 col-sm-12">
                                                            <label for="estadouser4" class="form-label">Estado</label>
                                                            <input required name="estadouser4" type="text" id="estadouser4" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="infoArea row">
                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="ruauser4" class="form-label">Rua</label>
                                                            <input required name="ruauser4" type="tect" id="ruauser4" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="completProcurador" class="form-label">Complemento</label>
                                                            <input name="completProcurador" type="text" id="completProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="numerocasaProcurador" class="form-label">Número</label>
                                                            <input required name="numerocasaProcurador" type="text" id="numerocasaProcurador" class="form-control">
                                                        </div>

                                                        <div class="col-md-3 col-sm-12">
                                                            <label for="bairrouser4" class="form-label">Bairro</label>
                                                            <input required name="bairrouser4" type="text" id="bairrouser4" class="form-control">
                                                        </div>
                                                    </div>
                                                    <div class="btnStatusProposta">
                                                        <button type="submit"  style="margin-left:auto;" class="cta">
                                                            <span class="hover-underline-animation">Adicionar</span>
                                                            <svg
                                                                id="arrow-horizontal"
                                                                xmlns="http://www.w3.org/2000/svg"
                                                                width="30"
                                                                height="10"
                                                                viewBox="0 0 46 16"
                                                            >
                                                                <path
                                                                id="Path_10"
                                                                data-name="Path 10"
                                                                d="M8,0,6.545,1.455l5.506,5.506H-30V9.039H12.052L6.545,14.545,8,16l8-8Z"
                                                                transform="translate(30)"
                                                                ></path>
                                                            </svg>
                                                        </button>
                                                    </div>
                                            </form>
                                        </div>
                                    </div>
                                </div>



                            </div>
                        <!--Final Tabela Procurador-->

                        </div>
                    </div>
            
            </div>
            
        </div>
    </div>
    </section>






    <footer>
        <p>©AprovaCredi 2024 | Todos os direitos reservados.<br>
                Site Elaborado por: <a target="_blank" href="https://www.linkedin.com/in/victor-hugo-ramiro-cota/"><b>Victor Hugo Ramiro Cota.</b></a>
    </p>
    </footer>
    <div id="whatswidget-pre-wrapper" class="">
    <div id="whatswidget-widget-wrapper" class="whatswidget-widget-wrapper" style="all:revert;">
    <div id="whatswidget-conversation" class="whatswidget-conversation" style="color: revert; font: revert; font-palette: revert; font-synthesis: revert; forced-color-adjust: revert; text-orientation: revert; text-rendering: revert; -webkit-font-smoothing: revert; -webkit-locale: revert; -webkit-text-orientation: revert; -webkit-writing-mode: revert; writing-mode: revert; zoom: revert; accent-color: revert; place-content: revert; place-items: revert; place-self: revert; alignment-baseline: revert; animation-composition: revert; animation: revert; app-region: revert; appearance: revert; aspect-ratio: revert; backdrop-filter: revert; backface-visibility: revert; background: revert; background-blend-mode: revert; baseline-shift: revert; baseline-source: revert; block-size: revert; border-block: revert; border: revert; border-radius: revert; border-collapse: revert; border-end-end-radius: revert; border-end-start-radius: revert; border-inline: revert; border-start-end-radius: revert; border-start-start-radius: revert; inset: revert; box-shadow: revert; box-sizing: revert; break-after: revert; break-before: revert; break-inside: revert; buffered-rendering: revert; caption-side: revert; caret-color: revert; clear: revert; clip: revert; clip-path: revert; clip-rule: revert; color-interpolation: revert; color-interpolation-filters: revert; color-rendering: revert; color-scheme: revert; columns: revert; column-fill: revert; gap: revert; column-rule: revert; column-span: revert; contain: revert; contain-intrinsic-block-size: revert; contain-intrinsic-size: revert; contain-intrinsic-inline-size: revert; container: revert; content: revert; content-visibility: revert; counter-increment: revert; counter-reset: revert; counter-set: revert; cursor: revert; cx: revert; cy: revert; d: revert; display: none; dominant-baseline: revert; empty-cells: revert; fill: revert; fill-opacity: revert; fill-rule: revert; filter: revert; flex: revert; flex-flow: revert; float: revert; flood-color: revert; flood-opacity: revert; grid: revert; grid-area: revert; height: revert; hyphenate-character: revert; hyphenate-limit-chars: revert; hyphens: revert; image-orientation: revert; image-rendering: revert; initial-letter: revert; inline-size: revert; inset-block: revert; inset-inline: revert; isolation: revert; letter-spacing: revert; lighting-color: revert; line-break: revert; list-style: revert; margin-block: revert; margin: revert; margin-inline: revert; marker: revert; mask: revert; mask-type: revert; math-depth: revert; math-shift: revert; math-style: revert; max-block-size: revert; max-height: revert; max-inline-size: revert; max-width: revert; min-block-size: revert; min-height: revert; min-inline-size: revert; min-width: revert; mix-blend-mode: revert; object-fit: revert; object-position: revert; object-view-box: revert; offset: revert; opacity: 0; order: revert; orphans: revert; outline: revert; outline-offset: revert; overflow-anchor: revert; overflow-clip-margin: revert; overflow-wrap: revert; overflow: revert; overscroll-behavior-block: revert; overscroll-behavior-inline: revert; overscroll-behavior: revert; padding-block: revert; padding: revert; padding-inline: revert; page: revert; page-orientation: revert; paint-order: revert; perspective: revert; perspective-origin: revert; pointer-events: revert; position: revert; quotes: revert; r: revert; resize: revert; rotate: revert; ruby-position: revert; rx: revert; ry: revert; scale: revert; scroll-behavior: revert; scroll-margin-block: revert; scroll-margin: revert; scroll-margin-inline: revert; scroll-padding-block: revert; scroll-padding: revert; scroll-padding-inline: revert; scroll-snap-align: revert; scroll-snap-stop: revert; scroll-snap-type: revert; scrollbar-gutter: revert; shape-image-threshold: revert; shape-margin: revert; shape-outside: revert; shape-rendering: revert; size: revert; speak: revert; stop-color: revert; stop-opacity: revert; stroke: revert; stroke-dasharray: revert; stroke-dashoffset: revert; stroke-linecap: revert; stroke-linejoin: revert; stroke-miterlimit: revert; stroke-opacity: revert; stroke-width: revert; tab-size: revert; table-layout: revert; text-align: revert; text-align-last: revert; text-anchor: revert; text-combine-upright: revert; text-decoration: revert; text-decoration-skip-ink: revert; text-emphasis: revert; text-emphasis-position: revert; text-indent: revert; text-overflow: revert; text-shadow: revert; text-size-adjust: revert; text-transform: revert; text-underline-offset: revert; text-underline-position: revert; touch-action: revert; transform: revert; transform-box: revert; transform-origin: revert; transform-style: revert; transition: revert; translate: revert; user-select: revert; vector-effect: revert; vertical-align: revert; view-transition-name: revert; visibility: revert; border-spacing: revert; -webkit-box-align: revert; -webkit-box-decoration-break: revert; -webkit-box-direction: revert; -webkit-box-flex: revert; -webkit-box-ordinal-group: revert; -webkit-box-orient: revert; -webkit-box-pack: revert; -webkit-box-reflect: revert; -webkit-highlight: revert; -webkit-line-break: revert; -webkit-line-clamp: revert; -webkit-mask-box-image: revert; -webkit-mask: revert; -webkit-mask-composite: revert; -webkit-print-color-adjust: revert; -webkit-rtl-ordering: revert; -webkit-ruby-position: revert; -webkit-tap-highlight-color: revert; -webkit-text-combine: revert; -webkit-text-decorations-in-effect: revert; -webkit-text-fill-color: revert; -webkit-text-security: revert; -webkit-text-stroke: revert; -webkit-user-drag: revert; -webkit-user-modify: revert; white-space: revert; widows: revert; width: revert; will-change: revert; word-break: revert; word-spacing: revert; x: revert; y: revert; z-index: revert;"><div class="whatswidget-conversation-header" style="all:revert;">
    <div style="text-aling:center;" id="whatswidget-conversation-title" class="whatswidget-conversation-title" style="all:revert;"> Atendimento Aprovacredi!</div></div><div id="whatswidget-conversation-message" class="whatswidget-conversation-message " style="all:revert;">Está precisando de ajuda?<br/> estamos disponiveis de <b>Segunda</b> a <b> Sexta</b> das <b>08h30</b> às <b>18h00</b> </div><div class="whatswidget-conversation-cta" style="all:revert;"> <a style="all:revert;" id="whatswidget-phone-desktop" target="_blank" href="https://wa.me/5513991080129?text=Ol%C3%A1%2C+sou+um+corretor+usuario+do+sistema+Aprovacredi+e+tenho+um+problema%2C+pode+me+ajudar%3F" class="whatswidget-cta whatswidget-cta-desktop">Enviar Mensagem</a> <a id="whatswidget-phone-mobile" target="_blank" href="https://wa.me/5513991080129?text=Ol%C3%A1%2C+eu+vim+pelo+Site+da+CP+Pré+moldados%2C+preciso+de+ajuda%21+" class="whatswidget-cta whatswidget-cta-mobile" style="all:revert;">Enviar Mensagem</a></div></div><div id="whatswidget-conversation-message-outer" class="whatswidget-conversation-message-outer" style="color: revert; font: revert; font-palette: revert; font-synthesis: revert; forced-color-adjust: revert; text-orientation: revert; text-rendering: revert; -webkit-font-smoothing: revert; -webkit-locale: revert; -webkit-text-orientation: revert; -webkit-writing-mode: revert; writing-mode: revert; zoom: revert; accent-color: revert; place-content: revert; place-items: revert; place-self: revert; alignment-baseline: revert; animation-composition: revert; animation: revert; app-region: revert; appearance: revert; aspect-ratio: revert; backdrop-filter: revert; backface-visibility: revert; background: revert; background-blend-mode: revert; baseline-shift: revert; baseline-source: revert; block-size: revert; border-block: revert; border: revert; border-radius: revert; border-collapse: revert; border-end-end-radius: revert; border-end-start-radius: revert; border-inline: revert; border-start-end-radius: revert; border-start-start-radius: revert; inset: revert; box-shadow: revert; box-sizing: revert; break-after: revert; break-before: revert; break-inside: revert; buffered-rendering: revert; caption-side: revert; caret-color: revert; clear: revert; clip: revert; clip-path: revert; clip-rule: revert; color-interpolation: revert; color-interpolation-filters: revert; color-rendering: revert; color-scheme: revert; columns: revert; column-fill: revert; gap: revert; column-rule: revert; column-span: revert; contain: revert; contain-intrinsic-block-size: revert; contain-intrinsic-size: revert; contain-intrinsic-inline-size: revert; container: revert; content: revert; content-visibility: revert; counter-increment: revert; counter-reset: revert; counter-set: revert; cursor: revert; cx: revert; cy: revert; d: revert; display: none; dominant-baseline: revert; empty-cells: revert; fill: revert; fill-opacity: revert; fill-rule: revert; filter: revert; flex: revert; flex-flow: revert; float: revert; flood-color: revert; flood-opacity: revert; grid: revert; grid-area: revert; height: revert; hyphenate-character: revert; hyphenate-limit-chars: revert; hyphens: revert; image-orientation: revert; image-rendering: revert; initial-letter: revert; inline-size: revert; inset-block: revert; inset-inline: revert; isolation: revert; letter-spacing: revert; lighting-color: revert; line-break: revert; list-style: revert; margin-block: revert; margin: revert; margin-inline: revert; marker: revert; mask: revert; mask-type: revert; math-depth: revert; math-shift: revert; math-style: revert; max-block-size: revert; max-height: revert; max-inline-size: revert; max-width: revert; min-block-size: revert; min-height: revert; min-inline-size: revert; min-width: revert; mix-blend-mode: revert; object-fit: revert; object-position: revert; object-view-box: revert; offset: revert; opacity: revert; order: revert; orphans: revert; outline: revert; outline-offset: revert; overflow-anchor: revert; overflow-clip-margin: revert; overflow-wrap: revert; overflow: revert; overscroll-behavior-block: revert; overscroll-behavior-inline: revert; overscroll-behavior: revert; padding-block: revert; padding: revert; padding-inline: revert; page: revert; page-orientation: revert; paint-order: revert; perspective: revert; perspective-origin: revert; pointer-events: revert; position: revert; quotes: revert; r: revert; resize: revert; rotate: revert; ruby-position: revert; rx: revert; ry: revert; scale: revert; scroll-behavior: revert; scroll-margin-block: revert; scroll-margin: revert; scroll-margin-inline: revert; scroll-padding-block: revert; scroll-padding: revert; scroll-padding-inline: revert; scroll-snap-align: revert; scroll-snap-stop: revert; scroll-snap-type: revert; scrollbar-gutter: revert; shape-image-threshold: revert; shape-margin: revert; shape-outside: revert; shape-rendering: revert; size: revert; speak: revert; stop-color: revert; stop-opacity: revert; stroke: revert; stroke-dasharray: revert; stroke-dashoffset: revert; stroke-linecap: revert; stroke-linejoin: revert; stroke-miterlimit: revert; stroke-opacity: revert; stroke-width: revert; tab-size: revert; table-layout: revert; text-align: revert; text-align-last: revert; text-anchor: revert; text-combine-upright: revert; text-decoration: revert; text-decoration-skip-ink: revert; text-emphasis: revert; text-emphasis-position: revert; text-indent: revert; text-overflow: revert; text-shadow: revert; text-size-adjust: revert; text-transform: revert; text-underline-offset: revert; text-underline-position: revert; touch-action: revert; transform: revert; transform-box: revert; transform-origin: revert; transform-style: revert; transition: revert; translate: revert; user-select: revert; vector-effect: revert; vertical-align: revert; view-transition-name: revert; visibility: revert; border-spacing: revert; -webkit-box-align: revert; -webkit-box-decoration-break: revert; -webkit-box-direction: revert; -webkit-box-flex: revert; -webkit-box-ordinal-group: revert; -webkit-box-orient: revert; -webkit-box-pack: revert; -webkit-box-reflect: revert; -webkit-highlight: revert; -webkit-line-break: revert; -webkit-line-clamp: revert; -webkit-mask-box-image: revert; -webkit-mask: revert; -webkit-mask-composite: revert; -webkit-print-color-adjust: revert; -webkit-rtl-ordering: revert; -webkit-ruby-position: revert; -webkit-tap-highlight-color: revert; -webkit-text-combine: revert; -webkit-text-decorations-in-effect: revert; -webkit-text-fill-color: revert; -webkit-text-security: revert; -webkit-text-stroke: revert; -webkit-user-drag: revert; -webkit-user-modify: revert; white-space: revert; widows: revert; width: revert; will-change: revert; word-break: revert; word-spacing: revert; x: revert; y: revert; z-index: revert;"> <span id="whatswidget-text-header-outer" class="whatswidget-text-header-outer" style="all:revert;">CP Pré Moldados</span><br> <div id="whatswidget-text-message-outer" class="whatswidget-text-message-outer" style="all:revert;">Precisa de ajuda? </div></div><div class="whatswidget-button-wrapper" style="all:revert;"><div class="whatswidget-button" id="whatswidget-button" style="all:revert;"><div style="margin:0 auto;width:38.5px;all:revert;"> <img class="whatswidget-icon" style="all:revert;" src=" data:image/png;base64,"></div></div></div>
    <script id="whatswidget-script" type="text/javascript">document.getElementById("whatswidget-conversation").style.display="none";document.getElementById("whatswidget-conversation").style.opacity="0"; var button=document.getElementById("whatswidget-button");button.addEventListener("click",openChat);var conversationMessageOuter=document.getElementById("whatswidget-conversation-message-outer");conversationMessageOuter.addEventListener("click",openChat);var chatOpen=!1;function openChat(){0==chatOpen?(document.getElementById("whatswidget-conversation").style.display="block",document.getElementById("whatswidget-conversation").style.opacity=100,chatOpen=!0,document.getElementById("whatswidget-conversation-message-outer").style.display="none"):(document.getElementById("whatswidget-conversation").style.opacity=0,document.getElementById("whatswidget-conversation").style.display="none",chatOpen=!1)}</script></div>
    <style id="whatswidget-style">.whatswidget-widget-wrapper{font-family:"Helvetica Neue","Apple Color Emoji",Helvetica,Arial,sans-serif !important;font-size:16px !important;position:fixed !important;bottom:-15px !important;right:45px !important;z-index:1001 !important}.whatswidget-conversation{background-color:#e4dcd4 !important;background-image:url('data:image/png !important;base64,') !important;background-repeat:repeat !important;box-shadow:rgba(0, 0, 0, 0.16) 0px 5px 40px !important;width:250px !important;height:300px !important;border-radius:10px !important;transition-duration:0.5s !important;margin-bottom:80px !important}.whatswidget-conversation-header{background-color:white !important;padding:10px !important;padding-left:25px !important;box-shadow:0px 1px #00000029 !important;font-weight:600 !important;border-top-left-radius:10px !important;border-top-right-radius:10px !important}.whatswidget-conversation-message{line-height: 1.2em !important;background-color:white !important;padding:10px !important;margin:10px !important;margin-left:15px !important;border-radius:5px !important}.whatswidget-conversation-message-outer{background-color:#FFF !important;padding:10px !important;margin:10px !important;margin-left:0px !important;border-radius:5px !important;box-shadow:rgba(0, 0, 0, 0.342) 0px 2.5px 10px !important;cursor:pointer !important;animation:nudge 2s linear infinite !important;margin-bottom:70px !important}.whatswidget-text-header-outer{font-weight:bold !important;font-size:90% !important}.whatswidget-text-message-outer{font-size:90% !important}.whatswidget-conversation-cta{border-radius:25px !important;width:175px !important;font-size:110% !important;padding:10px !important;margin:0 auto !important;text-align:center !important;background-color:#23b123 !important;color:white !important;font-weight:bold !important;box-shadow:rgba(0, 0, 0, 0.16) 0px 2.5px 10px !important;transition:1s !important;position:absolute !important;top:62% !important;left:10% !important}.whatswidget-conversation-cta:hover{transform:scale(1.1) !important;filter:brightness(1.3) !important}.whatswidget-cta{text-decoration:none !important;color:white !important}.whatswidget-cta-desktop{display:none !important}.whatswidget-cta-mobile{display:inherit !important}@media (min-width: 48em){.whatswidget-cta-desktop{display:inherit !important}
    .whatswidget-cta-mobile{display:none !important}}.whatswidget-button-wrapper{position:fixed !important;bottom:15px !important;right:15px !important}.whatswidget-button{position:relative !important;right:0px !important;background-color:#31d831 !important;border-radius:100% !important;width:50px !important;height:50px !important;box-shadow:2px 1px #0d630d63 !important;transition:1s !important}.whatswidget-icon{width:35px !important;height:35px !important;position:absolute !important; bottom:7px !important; left:8px !important;}.whatswidget-button:hover{filter:brightness(115%) !important;transform:rotate(15deg) scale(1.15) !important;cursor:pointer !important}@keyframes nudge{20%,100%{transform:translate(0,0)}0%{transform:translate(0,5px);transform:rotate(2deg)}10%{transform:translate(0,-5px);transform:rotate(-2deg)}}.whatswidget-link{position:absolute !important;bottom:90px !important;right:5px !important;opacity:0.5 !important}</style>
    </div>

    <script>
        $(function(){
            $('.formatReal').maskMoney({prefix:'R$ ', allowNegative: true, thousands:'.', decimal:',', affixesStay: true});
        })
    </script>

    <script src="../../../bootstrap/dist/js/bootstrap.bundle.min.js"></script>
    <script src="../../../javascript/buscarCEP.js"></script>
    <script src="../../../javascript/masksystem.js"></script>
    <script src="../../../javascript/sidebars.js"></script>
    <script src="../../../javascript/editarInpust.js"></script>
</body>
</html>
