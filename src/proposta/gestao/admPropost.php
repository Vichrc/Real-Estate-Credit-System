...
function validarValor($valor) {
    ...
    if (!empty($valor)) {
        // Pegar o primeiro caractere do valor
        $primeiroCaractere = $valor[0];
        
        // Verificar se o primeiro caractere é um número
        if (is_numeric($primeiroCaractere)) {
            return true;
        } else {
            return false;
        }
...
function diferencaDias($data) {
    ...
    // Convertendo a data para o formato aceito pela função strtotime
    $data_timestamp = strtotime(str_replace('/', '-', $data));

    // Obtendo o timestamp da data atual
    $data_atual = strtotime(date('Y-m-d'));

    // Calculando a diferença em segundos
    $diferenca_segundos = $data_atual - $data_timestamp;

    // Convertendo a diferença de segundos para dias
    $diferenca_dias = round($diferenca_segundos / (60 * 60 * 24));
...
$usuarioPrimario = $query_userPrimario->fetch_assoc();
    ...
    if(!isset($_GET['classNameBusc'])) {

        
        if($typeFilter == 'Aprovado') {

            $sql_MyPropost = "SELECT * FROM db_propostas WHERE status_proposta NOT LIKE '%Em Analise%' AND status_proposta NOT LIKE '%Reprovado%' AND status_proposta NOT LIKE '%Pre Aprovado%' AND status_proposta NOT LIKE 'Finalizado' AND status_proposta NOT LIKE 'Em Aberto' ORDER BY id_proposta DESC";
            $query_MyPropost = $mysqli->query($sql_MyPropost) or die($mysqli->error);
            $num_MyPropost = $query_MyPropost->num_rows;;
            
        } else {
        $sql_MyPropost = "SELECT * FROM db_propostas WHERE status_proposta = '$typeFilter'";
        $query_MyPropost = $mysqli->query($sql_MyPropost) or die($mysqli->error);
        $num_MyPropost = $query_MyPropost->num_rows;;
        }
        ...
        $resultado = validarValor($_GET['classNameBusc']);

        $buscador = $mysqli->real_escape_string($_GET['classNameBusc']);
        if($resultado === true) {
            $sql_MyPropost = "SELECT * FROM db_propostas WHERE cpf_comprador LIKE '%$buscador%'";
        } else {
            $sql_MyPropost = "SELECT * FROM db_propostas WHERE nome_comprador LIKE '%$buscador%'";
        }
        $query_MyPropost = $mysqli->query($sql_MyPropost) or die($mysqli->error);
        $num_MyPropost = $query_MyPropost->num_rows;
...
<html lang="pt-br">
  <head>
    ...
    <title>Aprovacredi - Criar Proposta</title>
                    ...
                    <ul class="nav nav-pills nav-flush flex-column mb-auto text-center">
                        ...
                        <div class="dropend">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li>
                                    <a href="../gestao/admPropost.php" class="active dropdown-item">
                                        <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                            ...
                            <div class="dropend">
                                <li class="nav-item dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        ...
                                        <li>
                                            <a href="admPropost.php" class=" active dropdown-item">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="margin-right:10px;" width="25" height="25" fill="currentColor" class="bi bi-house-door-fill" viewBox="0 0 16 16">
                                                    <path d="M1 0 0 1l2.2 3.081a1 1 0 0 0 .815.419h.07a1 1 0 0 1 .708.293l2.675 2.675-2.617 2.654A3.003 3.003 0 0 0 0 13a3 3 0 1 0 5.878-.851l2.654-2.617.968.968-.305.914a1 1 0 0 0 .242 1.023l3.27 3.27a.997.997 0 0 0 1.414 0l1.586-1.586a.997.997 0 0 0 0-1.414l-3.27-3.27a1 1 0 0 0-1.023-.242L10.5 9.5l-.96-.96 2.68-2.643A3.005 3.005 0 0 0 16 3c0-.269-.035-.53-.102-.777l-2.14 2.141L12 4l-.364-1.757L13.777.102a3 3 0 0 0-3.675 3.68L7.462 6.46 4.793 3.793a1 1 0 0 1-.293-.707v-.071a1 1 0 0 0-.419-.814L1 0Zm9.646 10.646a.5.5 0 0 1 .708 0l2.914 2.915a.5.5 0 0 1-.707.707l-2.915-2.914a.5.5 0 0 1 0-.708ZM3 11l.471.242.529.026.287.445.445.287.026.529L5 13l-.242.471-.026.529-.445.287-.287.445-.529.026L3 15l-.471-.242L2 14.732l-.287-.445L1.268 14l-.026-.529L1 13l.242-.471.026-.529.445-.287.287-.445.529-.026L3 11Z"/>
                                                </svg>    
                                                Gerenciamento de Propostas
    ...
    <section class="workspace">
        <div class="container container-workspace">
            ...
            <div class="headerArea">
                <h1><b>Gerenciar Proposta</b></h1>
                <div class="btn-conteiner">
                    <ul class="meuadmProposta row">
                        <li class="menuadmPro  <?php if($typeFilter == 'Em Aberto') { echo 'active';}; ?>"><a class="menuadmPro" href="admPropost.php?typeFilter=Em Aberto">Em Aberto</a></li>
                        <li class="menuadmPro <?php if($typeFilter == 'Pre Aprovado') { echo 'active';} ?>"><a href="admPropost.php?typeFilter=Pre Aprovado">Pre-Aprovados</a></li>
                        <li class="menuadmPro <?php if($typeFilter == 'Aprovado') { echo 'active';} ?>"><a href="admPropost.php?typeFilter=Aprovado">Aprovados</a></li>
                        <li class="menuadmPro <?php if($typeFilter == 'Finalizado') { echo 'active';} ?>"><a href="admPropost.php?typeFilter=Finalizado">Finalizados</a></li>
                        <li class="menuadmPro <?php if($typeFilter == 'Reprovado') { echo 'active';} ?>"><a href="admPropost.php?typeFilter=Reprovado">Reprovados</a></li>
                    </ul>
                </div>
                <form method="get">
                    <div class="containerpesquisa">
                        ...
                        <input name="classNameBusc" class="search_input" placeholder="Digite o nome ou CPF" type="text">
                        </div>
                    </div>
                </form>
            </div>
            <div class="container-table-proposta">
                <table class="table-proposta table">
                    <thead>
                        <tr>
                            <th style="text-align: center;" class="myPropost-hiden"></th>
                            <th >Nome/CPF</th>
                            <th class="myPropost-hiden" >Valor Financiado</th>
                            <th >Banco</th>
                            <th >Status</th>
                            <th style="text-align: center;" ></th>
                        </tr>
                    </thead>
                    <?php if($num_MyPropost == 0) { ?>
                                        <tr>
                                            <td colspan="6">Nem uma proposta encontrada!</td>
                                        </tr>
                    <?php } else { 
                        while($myPropost = $query_MyPropost->fetch_assoc()){

                            $idsec = $myPropost['id_usuario'];

                            $sql_userSec = "SELECT * FROM usuarios WHERE id  = $idsec";
                            $query_userSec = $mysqli->query($sql_userSec) or die($mysqli->error);
                            $num_userSec = $query_userSec->num_rows;
                            $userSec = $query_userSec->fetch_assoc();

                        ?>
                    <tbody>
                        <tr scope="row">
                            <?php 
                            $diferenca = diferencaDias($myPropost['data_criacao']);
                            switch ($diferenca) {
                                case ($diferenca == 1):
                            ?>
                                    <th class="myPropost-hiden" scope="col-1">
                                        <div class="containerimg">
                                            <div class="statusUser statusAccont">
                                                <div class="colorstatusPorpGreen"></div> 
                                            </div>
                                        </div>
                                    </th>
                            <?php
                            ...
                            ?>
                                    <th class="myPropost-hiden" scope="col-1">
                                        <div class="containerimg">
                                            <div class="statusUser statusAccont">
                                                <div class="colorstatusPropYellow"></div> 
                                            </div>
                                        </div>
                                    </th>
                            <?php
                            ...
                            ?>
                                <th class="myPropost-hiden" scope="col-1">
                                    <div class="containerimg">
                                        <div class="statusUser statusAccont">
                                            <div class="colorstatusPropred"></div> 
                                        </div>
                                    </div>
                                </th>
                            <?php
                            ...
                            ?>
                            <th class="textareaproposta">
                                <div class="nomeareaadm">
                                    <span><?php echo removerTextoDepoisDoEspaco($myPropost['nome_comprador']); ?></span>
                                    <span class=""><?php echo $myPropost['cpf_comprador']; ?></span>
                                </div>
                            </th>
                            <th class="myPropost-hiden textareaproposta">
                                <div class="resultarea">
                                    <span><?php echo $myPropost['financiado']; ?></span>
                                </div>
                            </th>
                            <th class="textareaproposta">
                                <div class="resultProposta">
                                    <span><?php echo $myPropost['banco_aprov']; ?></span>
                                </div>
                            </th>
                            <th class="textareaproposta">
                                <div class="resultProposta">
                                    <span><?php echo $myPropost['status_proposta']; ?></span>
                                </div>
                            </th>
                            <th class="edicaoProposta">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="gestaopropst.php?id=<?php echo $myPropost['id_proposta']; ?>">Visualisar proposta</a></li>
                                    <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalExcluirProposta<?php echo $myPropost['id_proposta'];?>">Excluir Proposta</a></li>
                                </ul>
                            </th>
                        </tr>
                    </tbody>

                    <div class="modal fade" id="modalExcluirProposta<?php echo $myPropost['id_proposta'];?>" tabindex="-1" aria-labelledby="labelmodalExcluirProposta<?php echo $myPropost['id_proposta'];?>" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="labelmodalExcluirProposta<?php echo $myPropost['id_proposta'];?>">Excluir proposta</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <p style="color:red;text-align:center;"><b>você esta prestes a excluir a proposta de <?php echo $myPropost['nome_comprador'];?>, gostaria de processeguir?</b></p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                                <a href="deletPropost.php?id=<?php echo $myPropost['id_proposta']; ?>">
...
<script>
// Função para enviar o valor do input quando o usuário terminar de digitar
function enviarValorInput() {
    // Obtém o valor do input
    var valorInput = document.getElementById('searchInput').value;
    
    // Aqui você pode enviar o valor para onde desejar, por exemplo, via AJAX
...
document.getElementById('searchInput').addEventListener('keyup', function(event) {
    ...
    if (event.keyCode === 13) {
        // Se for, chama a função para enviar o valor do input
        enviarValorInput();
    }
});
</script>
    ...
    <script src="../../../javascript/validationCractProp.js"></script>
</body>
</html>
