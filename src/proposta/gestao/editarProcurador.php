...
$pasta = "/important/docProcurador/";
$pasta2 = "../../../important/docProcurador/";
...
$id_proposta = $dadosProcurador['id_proposta'];

$sql_docProcurador = "SELECT * FROM db_doc_proc WHERE id_procurador = $id_procurador";
$query_docProcurador = $mysqli->query($sql_docProcurador) or die($mysqli->error);
$num_docProcurador = $query_docProcurador->num_rows;
$docProcurador = $query_docProcurador->fetch_assoc();


    if($usuarioPrimario['permissao_usuario'] != 3){

        if($usuarioPrimario['permissao_usuario'] != 2){
            if($usuarioPrimario['id'] != $dadosProcurador['id_corretor']) {
                header("Location: ../../../logout.php");
                exit();
            }
        }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES)) {
        ...
        if(isset($_FILES['cpf_file'])) {
            $idusuario = $id_procurador;
            $nomeArquivoNovo = "comp_".$id_procurador."_".uniqid();
            ...
            if($extensao_Arqv_cpf != ''){
                ...
                if ($num_dadosdocProcurador > 0) {
                    if($deu_certor) { $mysqli->query(
                        "UPDATE db_doc_proc 
                        SET path_cpf = '$path' 
                        WHERE id_procurador = '$id_procurador'")  or die($mysqli->error);
                    if($deu_certor) echo "<script>window.location.href = 'editarVendedor.php?id=$id_procurador'</script>"; else echo "<script>alert('ERRO: falha inesperada ao enviar arquivo.'); history.go(-1);</script>";
                    }
                } else {
                    $sql_code = "INSERT INTO db_doc_proc (id_corretor, id_proposta, id_procurador, path_cpf) 
                    VALUE ('$idUser' ,'$id_proposta' ,'$id_procurador' , '$path')";
        ...
        if(isset($_FILES['rg_cnh_file'])) {
            ...
            if($extensao_Arqv_rg_cnh != ''){
                ...
                if ($num_dadosdocProcurador > 0) {
                    ...
                    $sql_code = "INSERT INTO db_doc_proc (id_corretor, id_proposta, id_procurador, path_rg_cnh) 
                    VALUE ('$idUser' ,'$id_proposta' ,'$id_procurador' , '$path')";
                    $deu_certo = $mysqli->query($sql_code) or die($mysqli->error);
                }
            }
        ...
        if(isset($_FILES['procu_file'])) {
            $idusuario = $id_procurador;
            $nomeArquivoNovo = "comp_".$id_procurador."_".uniqid();
            $arq_Upload = $_FILES['procu_file'];
            $arquivo = $arq_Upload ['name'];
            ...
            if($extensao_Arqv_Compr_resi != ''){
                ...
                if ($num_dadosdocProcurador > 0) {
                    ...
                    $sql_code = "INSERT INTO db_doc_proc (id_corretor, id_proposta, id_procurador, path_procu) 
                    VALUE ('$idUser' ,'$id_proposta' ,'$id_procurador' , '$path')";
                    $deu_certo = $mysqli->query($sql_code) or die($mysqli->error);
                }
            }
...
<body>
    ...
    <section class="workspace">
        <div class="container container-workspace containerdados">
            ...
            <div class="areaInicial">
                <div class="retorno">
                    <a href="gestaopropst.php?id=<?php echo $dadosProcurador['id_proposta'] ?>"><i class="fi fi-sr-undo"></i> Voltar</a>
                </div>
                <div class="btnArea">
                    <button class="btnGestaoProposta excluir" data-bs-toggle="modal" data-bs-target="#modalDeletarProcurador"> 
                        <i class="fi fi-ss-trash-xmark"></i></i> <span class="btngrupDados">Excluir</span> 
                    </button>

                    <button class="btnGestaoProposta" data-bs-toggle="modal" data-bs-target="#documentoProcurador"> 
                        <i class="fi fi-sr-add-document"></i> <span class="btngrupDados">Documentos</span> 
                    </button>

                </div>
            </div>
            <form action="editarDadosProcurador.php?id=<?php echo $id_procurador ?>" method="post"> 
                ...
                <div class="infoArea row">
                    <div class="col-md-6 col-sm-12">
                        ...
                        <input required value="<?php echo $dadosProcurador['nome_procurador'];  ?>" name="nameProcurador" type="text" id="nameProcurador" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label for="nascimentoProcurador" class="form-label">Nascimento</label>
                        <input required value="<?php echo $dadosProcurador['data_nascimento_procurador'];  ?>" name="nascimentoProcurador" type="date" id="nascimentoProcurador" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label class="form-label">Estado Civil</label>
                        <select name="estadoCivilProcurador" class="form-select" aria-label="Default select example">
                            <option value="<?php echo $dadosProcurador['estado_civil_procurador'];  ?>"><?php echo $dadosProcurador['estado_civil_procurador'];  ?></option>
                ...
                <div class="infoArea row">
                    <div class="col-md-8 col-sm-12">
                        ...
                        <input required value="<?php echo $dadosProcurador['email_procurador'];  ?>" name="emailProcurador" type="email" id="emailProcurador" class="form-control">
                    </div>

                    <div class="col-md-4 col-sm-12">
                        <label for="telefoneProcurador" class="form-label">Telefone</label>
                        <input required value="<?php echo $dadosProcurador['telefone_procurador'];  ?>" name="telefoneProcurador" type="text" id="telefoneProcurador" class="form-control telefone">
                    </div>
                </div>
                <div class="infoArea row">
                    <div class="col-md-3 col-sm-12">
                        <label for="rgProcurador" class="form-label">RG</label>
                        <input required value="<?php echo $dadosProcurador['documento_procurador'];  ?>" name="rgProcurador" type="text" id="rgProcurador" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label for="emissorProcurador" class="form-label">Orgão Emissor</label>
                        <input required value="<?php echo $dadosProcurador['org_emissor'];  ?>" name="emissorProcurador" type="text" id="emissorProcurador" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label for="dataEmissaoProcurador" class="form-label">Data Emissor</label>
                        <input required value="<?php echo $dadosProcurador['data_emissao'];  ?>" name="dataEmissaoProcurador" type="date" id="dataEmissaoProcurador" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label for="cpfProcurador" class="form-label">CPF</label>
                        <input required value="<?php echo $dadosProcurador['cpf_procurador'];  ?>" name="cpfProcurador" type="text" id="cpfProcurador" class="form-control cpfMask">
                    </div>
                </div>
                <div class="infoArea row">
                    <div class="col-md-6 col-sm-12">
                        <label for="nomeMaeProcurador" class="form-label">Nome da Mãe</label>
                        <input required value="<?php echo $dadosProcurador['nome_mae_procurador'];  ?>" name="nomeMaeProcurador" type="tect" id="nomeMaeProcurador" class="form-control">
                    </div>

                    <div class="col-md-6 col-sm-12">
                        <label for="nomepaiProcurador" class="form-label">Nome do Pai</label>
                        <input value="<?php echo $dadosProcurador['nome_pai_procurador'];  ?>" name="nomepaiProcurador" type="text" id="nomepaiProcurador" class="form-control">
                    </div>
                </div>
                ...
                <div class="infoArea row">
                    <div disabled class="col-md-4 col-sm-12">
                        ...
                        <input required value="<?php echo $dadosProcurador['cep_procurador'];  ?>" name="cepInput4" type="tect" id="cepInput4" class="form-control cepInput">
                    </div>

                    <div disabled class="col-md-4 col-sm-12">
                        <label for="cidadeuser4" class="form-label">Cidade</label>
                        <input required value="<?php echo $dadosProcurador['cidade_casa_procurador'];  ?>" name="cidadeuser4" type="text" id="cidadeuser4" class="form-control">
                    </div>

                    <div class="col-md-4 col-sm-12">
                        <label for="estadouser4" class="form-label">Estado</label>
                        <input required value="<?php echo $dadosProcurador['estado_casa_procurador'];  ?>" name="estadouser4" type="text" id="estadouser4" class="form-control">
                    </div>
                </div>
                <div class="infoArea row">
                    <div class="col-md-3 col-sm-12">
                        <label for="ruauser4" class="form-label">Rua</label>
                        <input required value="<?php echo $dadosProcurador['endereço_procurador'];  ?>" name="ruauser4" type="tect" id="ruauser4" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label for="completProcurador" class="form-label">Complemento</label>
                        <input value="<?php echo $dadosProcurador['complemento_casa_procurador'];  ?>" name="completProcurador" type="text" id="completProcurador" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label for="numerocasaProcurador" class="form-label">Número</label>
                        <input required value="<?php echo $dadosProcurador['numero_casa_procurador'];  ?>" name="numerocasaProcurador" type="text" id="numerocasaProcurador" class="form-control">
                    </div>

                    <div class="col-md-3 col-sm-12">
                        <label for="bairrouser4" class="form-label">Bairro</label>
                        <input required value="<?php echo $dadosProcurador['bairro_casa_procurador'];  ?>" name="bairrouser4" type="text" id="bairrouser4" class="form-control">
                    </div>
                </div>
    ...
    <!-- Modal Deletar Procurador -->
    <div class="modal fade" id="modalDeletarProcurador" tabindex="-1" aria-labelledby="modalDeletarProcuradorLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="modalDeletarProcuradorLabel">Excluir Procurador</h1>
            ...
            <div class="modal-footer">
                ...
                <a href="deletProcurador.php?id=<?php echo $dadosProcurador['id_procurador'];?>">
    ...
    <!-- Modal Documentos -->
    <div class="modal fade" id="documentoProcurador" tabindex="-1" aria-labelledby="documentoProcuradorLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="documentoProcuradorLabel">Documentos de <?php echo removerTextoDepoisDoEspaco($dadosProcurador['nome_procurador']); ?></h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
                <form enctype="multipart/form-data" method="POST">
                    <div class="modal-body">  

                            <?php if(!isset($docProcurador['path_rg_cnh']) || $docProcurador['path_rg_cnh'] == '') { ?>
                            ...
                            <?php if(!isset($docProcurador['path_cpf']) || $docProcurador['path_cpf'] == '') { ?>
                                <div class="inputforms">
                            ...
                            <?php if(!isset($docProcurador['path_procu']) || $docProcurador['path_procu'] == '' ) { ?>
                                <div class="inputforms">
                                    <label class="" ><b>Procuração Vigente</b></label>
                                    <div class="input-group">
                                        <input name="procu_file" type="file" class="form-control" id="">
                                    </div>
                                </div>
                            <?php } else { ?>
                                <div style="margin: 15px 0;text-align:center;">                                    
                                    <span style="color:green;"><b>Procuração Vigente Já enviado!!!</b></span>
                                </div>
                            <?php } ?>
                    </div>
                    <div class="modal-footer">
